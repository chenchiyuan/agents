...
const CHAT_COLUMNS = 'chat_id, title, agent_id, state, created_at, updated_at, closed_at';
...
const LIST_WITH = 'WITH p(q, agent, state, from_ts, to_ts) AS (VALUES (?, ?, ?, ?, ?)) ';
...
export function openDb(dbPath) {
  ...
  const stmts = {
    ...
    setOutputState: db.prepare(`UPDATE chats SET state = ?, updated_at = ? WHERE chat_id = ? AND state != 'closed'`),
    ...
    listChats: db.prepare(
      `${LIST_WITH}SELECT c.chat_id, c.title, c.agent_id, c.state, c.created_at, c.updated_at,
  ...
  function listChats({ q, agent, state, from, to, limit = LIMIT_DEFAULT, offset = 0 } = {}) {
    ...
    const params = [qN === null ? null : `%${escapeLike(qN)}%`, readOptionalString(agent, 'agent'), stateN, fromN, toN];
  ...
  return { insertInput, insertOutput, upsertChat, closeChat, startupSweep, listChats, getChat, close: () => db.close() };
}
