...
数值类 env 一律要求正整数，非法值启动即报错退出（快速失败）。
...
- **顶栏入口**：「文档」→ `/docs`（字段级接口文档页：方法 / 路径 / 参数 / 请求体字段 / 响应形态 / 错误码，结构与分发来自路由登记，不含示例）；「调试」→ `/debug`（按登记生成表单、真实发送并显示墙钟耗时，写操作发送前需确认）
- **顶栏入口 · 调用**：「调用」→ `/calls`（调用 roster：调用 id / agent / 状态 / 开始时间 / 结束时间 / 模型，六列；进入取一次 + 5 s 轮询刷新，点击某行按该调用订阅 SSE 进度）
...
- **对话状态**：`working` → `completed`（成功）/ `failed`（失败轮或派发失败）；关闭后为 `closed`。
  ...
  手动改名不改变对话状态与归档标记，也不刷新最近更新时间（列表位置不变）。
...
- **字段级接口文档页**：<http://127.0.0.1:7788/docs>（结构与分发来自路由登记，页面加载时取 `GET /api/docs` 的投影；参数语义说明为人工撰写，叙述与示例职责归 `API.md`）
- **可交互调试台**：<http://127.0.0.1:7788/debug>（按登记生成表单、真实发送并显示墙钟耗时；写接口发送前有一次显式确认，接口列表与表单两处带危险标识）
- **AI 索引文件**：<http://127.0.0.1:7788/llms.txt>（仓库内快照 `oamp/llms.txt`，落包根；HTTP 产物与快照是同一份字节）
- **重新生成索引快照**：改动路由登记后，在 `oamp/` 目录执行 `node scripts/gen-llms-txt.mjs`
- **漂移锁**：元数据必填 / 索引快照逐字节 / `API.md` 路径登记三条锁
