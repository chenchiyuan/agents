...
**已裁决 D7（用户）**：**约定不重启**——把"任务期间不得重启 hub / agent 进程"写成**运行约束**，**不投持久化**。配套两条：
- 协议**写明**："hub 重启 ⇒ 调用登记与收件箱丢失（`epoch` 变更）；客户端按 P-C′ 快照重建"，**不静默**；
- 因此 §三 P-E.3 的"hub 自身可用性"由**运维约束 + 显式声明**承担，不由实现承担（本迭代**零改动**）。

**已裁决（用户 2026-09-16）**：D1 客户端会话 ✓ | D2 收件箱内存实现 + 写明重启即丢 ✓ | D4 取消复用 `failed+error` ✓ | D6 最小 mock 身份、协议留扩展点 ✓ | **D7 约定不重启（不投持久化）✓** | Q1 **SSE 优先** ✓ | Q2 **服务化** ✓ | Q3 **直接做完整闭环** ✓ | **需求口径：实时流可丢 ✓ / 续接必须可靠 ✓ / hub 与 hub↔agent 控制面不可丢 ✓ / 一轮快照可见 ✓**
...
4. **控制面不丢**：客户端断线期间派发/取消/队列照常工作；agent 断线重连后状态收敛一致。**hub 自身可用性由运行约束承担**（D7：任务期间不重启），协议写明"重启 ⇒ 调用面与收件箱丢失（`epoch` 变更）⇒ 快照重建"，不静默。
