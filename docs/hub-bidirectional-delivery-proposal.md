...
|---|---|
| 流式增量帧（`chunk` / 中间 `call_update` / 心跳） | **hub 的可用与恢复能力**（重启后可重连、可重新纳管 agent、可恢复到可控可调用） |
...
3. **hub 自身的可用性与恢复**：见 **P-G**（D7′ 已裁决为"重启可恢复"：客户端可重连、agent 可重连、系统回到可控可调用；重启前的调用登记不保证，以产物/DB 为准）。
...
### P-G · 重启恢复协议（**"重启后可重连并恢复可控"**）

**保证与不保证（明说）**

| 保证（纳入验收） | 不保证（写进协议，不静默） |
|---|---|
| hub 重启后，**客户端能重连**（身份不变、可继续派发与查询） | 重启前的**调用登记**（in-flight 调用与未取结果指针）——内存，随代次丢失 |
| hub 重启后，**agent 能重连回来**（或由 `cluster up` 重建），并回到在线可派发 | 重启窗口内的实时事件（本就 best-effort） |
| 恢复完成后系统回到 **"可派发 / 可查询 / 可控"** 状态，且有**判据可查** | hub 对"重启前已发生什么"的记忆——**以产物与 DB 为准**（见下） |

**四条契约**

1. **agent → hub 重连**（现状已具备，补可观测）：`reconnect` **默认开启**且有指数退避（`src/config.js:139-152`、`src/agent.js:823`）；本迭代补的是**状态可观测**——`GET /api/agents` 必须能区分 `online` / **`reconnecting`** / `offline`，否则客户端无法分辨"agent 死了"与"正在重连"。
2. **客户端 → hub 重连**：身份 `principal_id` 为**幂等 upsert**（P-A）⇒ 重连后身份不变、无需重新协商；重连流程固定为 **`epoch` 校验 → 一轮快照 → 继续订阅/调度**（P-C′）。
3. **恢复判据（服务化）**：新增 `GET /api/health`（或扩 `hub doctor` 的输出）回答三问：**router 起没起 / web 起没起 / 有几个 agent 在线（含重连中）**；并以 `cluster up --wait` 的既有语义作为"恢复到可调用"的达成条件（现已是 10 实例全 `online` 才返回）。
4. **权威归属条文（本次最关键的一条）**：
   > **任务成果的权威源是磁盘产物与 DB（对话消息），hub 的调用登记只是易失的实时视图。**
   > 因此重启后"继续任务"的判据是 **产物对账 + 快照**，而不是"hub 记得"。这条把本仓既有的「产物是唯一输出契约」升格为**恢复协议的一部分**：hub 丢了记忆不要紧，产物在就还能继续。

**验收判据**
- **软重启**（只重启 hub，保留 agent 进程）⇒ agent 自动重连回来，`agents` 快照在可接受时间内回到全 `online`（期间可见 `reconnecting`）。
- **硬重启**（`cluster down` + `cluster up --wait`）⇒ 恢复判据转绿（router/web 可达、10 实例在线），客户端**不改任何身份**即可继续派发。
- **客户端侧断线**（用户场景："我断掉之后能拉起继续任务"）⇒ 拉起后 **一轮快照 + 产物对账**即可回答"当前进度与状态"，并继续调度；**无需回放**。
...
**已裁决 D7′（用户纠正）**：**不是"约定不重启"，而是"重启后可恢复"**——用户原话：
> "不能重启的表述也不对，应该是**重启之后能重新连接**。也就是**能重连 hub，hub 也能重连 agent**。**整个系统恢复控制可调用状态**。"

⇒ 本迭代**不靠"不重启"回避**，而是把**恢复链**做成契约与可观测能力（详见 §三 P-G）。

**已裁决（用户 2026-09-16）**：D1 客户端会话 ✓ | D2 收件箱内存实现 + 写明重启即丢 ✓ | D4 取消复用 `failed+error` ✓ | D6 最小 mock 身份、协议留扩展点 ✓ | **D7′ 重启可恢复（重连 hub / hub 重连 agent / 恢复可控可调用）✓** | Q1 **SSE 优先** ✓ | Q2 **服务化** ✓ | Q3 **直接做完整闭环** ✓ | **需求口径：实时流可丢 ✓ / 续接必须可靠 ✓ / 控制面与恢复能力不可丢 ✓ / 一轮快照可见 ✓**

**待默认值（未反对即生效）**：D3 收件箱保留期 = 跟随状态寿命；D5 自派发 = 先告警不硬拒。
...
| 7 | **P-G 重启恢复协议** | `GET /api/health`（router/web/agents 三项判据）、`agents` 增 `reconnecting` 状态、恢复流程条文 |
...
4. **控制面与恢复不丢**：客户端断线期间派发/取消/队列照常工作；agent 断线重连后状态收敛一致；**hub 重启（软/硬）后系统回到"可派发 / 可查询 / 可控"**，且有判据可查（P-G）；重启前的调用登记不保证，但**结果在 DB、成果在产物**，以对账继续（明文，不静默）。
