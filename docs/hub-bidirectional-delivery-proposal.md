...
### P-C″ · 接缝语义（补拉与实时的交界；**"续上之后新消息还能读到吗"**）

**答：能读，而且必须保证。** 关键是把"新的"与"旧的"放进**同一条有序流**、用**同一个游标序**：

1. **单流形态（推荐）**：SSE 端点支持 `?since=<cursor>` / `Last-Event-ID` ⇒ 服务端**先从收件箱日志回放**（补旧洞），**再在同一条连接上继续实时推送** ⇒ 补拉与实时**无接缝**，客户端看到的是一条连续的 cursor 序。
2. **双调形态（若分成"拉收件箱 + 订阅"两步）**：契约强制 **"先订阅、后补拉"**，客户端按 cursor 单调性丢弃 `≤ 当前游标` 的帧 ⇒ **不丢（订阅先建立，覆盖补拉窗口）+ 不重（cursor 去重）**。
3. **`ack` 的边界**：`ack` 只决定"收件箱是否继续保留该条"，**不影响实时流**；未 ack 的重投由 `message_id`/cursor 去重 ⇒ 语义是 **"宁可重复，不可丢失"**。
4. **保留窗的边界**：保留窗只约束"**能补多久的历史**"，**不约束"新的能否读到"**——实时路径与保留窗无关（这是最容易被误解的一点）。
5. **多读者游标分账**：游标按 `(principal, reader_id)` 计（缺省 reader = `default`），避免同一 principal 的两个消费者互相推进/抢游标。
6. ⇒ 对"续接之后新增信息是仍然能读到对吧"的正式回答：**是**——新的走实时路径（或同一流里游标更大的帧），与补历史共用一条流、一个游标序；唯一不可续的情形是 **`epoch` 变更**（hub 重启），此时按 §P-C′ 第 4 条显式告知并快照重同步，**重同步完成后的新消息照常实时可达**。
