...
- `oamp/test/call-protocol.test.js`（**修改**：agent 启动点注入同款 env（检索式 `agentEnv`、`OAMP_OMP_BIN`）；该文件的桩为 ACP 形态，且 `组 F` 断言 `call_update.kind ∈ {chunk, stdout, stderr}`（检索式 `['chunk', 'stdout', 'stderr'].includes`），故其绿依赖「常驻链路仍是 acp」这一前提被显式固定）
  - **D-1 登记（测试面归属口径）**：`architecture.md` §9.4.1 的「既有测试面 · 最小更新」列 6 个文件，**实测应为 7 个**——测试面 ACP-only 桩文件的全体 = `acp-daemon` / `call-protocol` / `confirmation-roundtrip` / `context-pool` / `project-workspace` / `tool-permission` / `web`（`FAKE_ACP_SOURCE` + `msg.method` 桩分布实测）。`call-protocol.test.js` **属测试面**，由本 PR 固定；同清单的 `tool-permission` / `confirmation-roundtrip` / `web` 三个因随实现改造而变（直引 `AcpClient` / `ContextPool` / 默认路径断言），归 pr-003。`architecture.md` §9.4.1 的清单回填由主 agent 另行处置（本 PR 只登记正确口径）。

**零改动（防夹带）**：`oamp/src/**` 全部（本 PR 不含任何生产变更）；`oamp/test/helpers/**`（既有公共 harness 不改，注入走各文件既有的 `envExtra` / `env` 通道）；其余既有测试文件（`oamp/test/*.test.js` 实测 **29** 个，扣除本 PR 的 4 个后 **25** 个；`oamp/test/` 下 `.js` 共 31 个，另含 `helpers/harness.js`、`helpers/fake-node.js`）。

## 验收标准

- [ ] 上述 4 个文件的所有 agent 启动点均显式传入 `OAMP_PROTOCOL: 'acp'`（逐文件 `grep -n "OAMP_PROTOCOL" oamp/test/<file>` 命中且落点是 agent 启动 env）。
- [ ] argv 期望值不再写死「内置即 acp」：改为按 `launcher.js` 的 `PROFILES['omp:acp']` / `PROFILES['omp:oneshot']` 期望值断言——**直接引用 pr-001 的 `PROFILES` 表**，不得以本地复写的「一致期望数组」替代（那会使本 PR 对 pr-001 的依赖在实现层可被绕过、退化为顺序偏好）；四条既有硬约束逐字保持：常驻 argv 首段 `acp`、工具开关与 `--no-tools` 同向、`--append-system-prompt <角色 md 绝对路径>`、档位 `--approval-mode` 在「常驻 tools on」= `always-ask` /「一次性」= `yolo`。
...
**择一判定声明（D-6 · 跨 PR 验收归属）**：F06 验收 1（acp 既有行为不退化）由 `prs/pr-003-protocol-layer-and-consumption-cutover.md` 判**主面（行为回归面）**；本 PR 只判**辅面（测试面前置）**——把 harness 的「内置默认即 acp」隐式假设显式固定为 `OAMP_PROTOCOL='acp'`，使该回归判据在常驻链路默认切成 rpc 后仍可判。本 PR 不对 acp 行为本身下断言（生产源码零改动）；同一条验收只计一次主面判定、不互推。
...
- `pr-001-launcher-and-protocol-config.md`（理由：本 PR 的 argv 期望值真源 = pr-001 新建的 `oamp/src/launcher.js` 的 `PROFILES` 表；注入键 `OAMP_PROTOCOL` 由 pr-001 的 `oamp/src/config.js` 交付；**依赖强度已锁定**（验收标准第 2 条不得以本地复写的期望数组替代，两条理由均为代码级产出、非文档叙述顺序）：断言直接引用 `PROFILES['omp:acp']` / `PROFILES['omp:oneshot']` 的键值，pr-001 未合并时该表不存在 ⇒ 本 PR 的判据无法成立。现状真源可指：`oamp/src/acp-client.js:137`（`['acp','--no-skills','--no-rules']`）、`oamp/src/agent.js:190-199`（`['-p','--no-session']`）即本 PR 所引期望值的今天形态。）
