...
**报告文件**: `docs/iterations/0029-hub-client-session-and-duplex/clarifications/verify-20260916-162032-pr-001-router-status-primitives.md`
**验证日期**: 2026-09-16
...
产出物：`feat/0029-pr-001-router-status-primitives` @ `fe67383`（工作区 `/Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0029-hub-client-session-and-duplex/.pb-agents/worktrees/0029-pr-001-router-status-primitives`；merge-base `72b659f`）
...
- **标准 1 · 验收标准逐条判定**：pass
  ...
  **1.1 generation（第 28 行）**：pass
  ```text
  GEN_STABLE true string 4c29d360-b3c8-4fab-b34a-e22e68e010e1
  GEN_RESTART_CHANGED true 6c17096f-db34-4723-b794-8588ad393936
  ```

  **1.2 connected + COLUMNS（第 29 行）**：pass
  ```text
  NODE_KEYS instance_id,session_id,state,last_heartbeat,connected
  COLUMNS_EQ true   （status.js COLUMNS 与 72b659f 逐字相同）
  ```
  ...
  **1.4 started_at（第 31 行）**：pass
  ```text
  WORKING_ROW {"state":"working","started_at":1789546923937,"keys":["task_id","from","to","state","label","created_at","started_at","updated_at","updates","updatesTruncated","model"]}
  ```
  既有 list 键保留，仅追加 `started_at`。
  ...
  **1.9 零依赖（第 36 行）**：pass
  `git diff --name-only 72b659f..HEAD` 的 `oamp/` 仅 `registry.js` `router.js` `uds.js`；`package.json`/`persist.js` 空。

  **1.10 八方法改动前后（第 37 行）**：pass
  本轮对 `git archive 72b659f` 与当前 worktree 各起 Router，同一入参：
  ```text
  register_keys 相同
  send INVALID_MESSAGE / ack UNKNOWN_MESSAGE 相同
  task_get {task:null} / task_list 数组 / deregister {removed:true} 相同
  node 既有 4 键相等（OLD4_EQ true）
  STATUS_TOP_NOW generation,nodes  BASE nodes   （仅允许的追加）
  ```

- **标准 2 · 改动面封闭性**：pass
  name-only：`registry.js` `router.js` `uds.js` + 本 PR 文件 + `pr-001-…-tasks.md`。
  不触碰清单零命中。`*-tasks.md` 按本委托口径不构成夹带。

- **标准 3 · 验收证据原文照录**：fail
  第 59–187、191–413 行虽有 `$` 前缀，但：
  1. 命令入口是仓库外 `/tmp/pr001-functional-compact.mjs`、`/tmp/pr001-generation.mjs`、`/tmp/pr001-probe.mjs`、`/tmp/pr001-compare.mjs`——读者无法从仓库复现。
  2. 贴出的 JSON 含占位符 `"session_id": "<session_id>"`、`"<last_heartbeat>"`（第 204、207、241–243、282 行等），不是当次 stdout 原样。
  3. 第 417–419 行两条 `git diff` **无任何输出块**。
  对照标准「可执行、可复核、不得含转述/摘要」：脚本不在仓内 + 占位替换 = 不可复核。

- **标准 4 · 既有面零影响（Z-3 / Z-7）**：pass
  `finishTask` 体与基线空白归一全等；节点 4 字段顺序不变；`status.js` COLUMNS 不变；八方法既有形状与基线 `OLD4_EQ true`。
...
| 规格/文档描述 | 实现实际行为 | 建议处理 |
|---|---|---|
| 第 34/43 行锚点 `registry.js:238` = finishTask | 现测 `finishTask` 约在 `:244` | 按实现更新文档 |
| 证据 JSON 用 `<session_id>` 替换 | 非原样 stdout | 按实现更新文档：贴未替换输出，或把脚本纳入仓库 |
| 第 417–419 行 git 命令无输出 | 无法核对是否空 diff | 按实现更新文档：附空输出或 `echo exit=0` |

## 下一迭代候选

- 验收证据脚本应落在迭代产物或 PR 文件内，避免 `/tmp` 一次性路径。

## 结论

**FAIL**（有 fail 条目）

功能面本轮独立实跑全部成立（含基线八方法对照）。阻塞交付的是标准 3：证据仍不可从仓库复现，且含占位符。主 agent 需把脚本入库或改为仓内可复制命令，并去掉 `<session_id>` 替换。
