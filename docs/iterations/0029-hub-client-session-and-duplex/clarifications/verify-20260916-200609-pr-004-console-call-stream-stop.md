# 阶段 5 独立验证报告 · pr-004-console-call-stream-stop

**报告文件**: `docs/iterations/0029-hub-client-session-and-duplex/clarifications/verify-20260916-200609-pr-004-console-call-stream-stop.md`
**验证日期**: 2026-09-16
...
验证者身份：同级代码审查者（控制台调用进度 EventSource 终态关流）
产出物：`feat/0029-pr-004-console-call-stream-stop` @ `45027a6`（工作区 `/Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0029-hub-client-session-and-duplex/.pb-agents/worktrees/0029-pr-004-console-call-stream-stop`；merge-base `cfb6736`）
验证标准来源：本委托 4 条 + PR「验收标准」第 23–29 行 7 款 + `architecture.md` §6.2 S-2 / S-9；标准 2 口径：`*-tasks.md` 不构成夹带
验证日期：2026-09-16

说明：不采信「验收证据」段自述。未改被验证产物。未对 :7788 集群做 up/down / `calls create`。浏览器探针使用独立端口，只托管本 worktree 的 `calls.html` / `calls.js`。

## 逐项判定

- **标准 1 · 验收标准逐条判定**：partial
  通过：第 23–25、27–29 行。不通过拆开：第 26 行「真集群 smoke（Network 面板无每秒重连）」本轮未在运行中的 :7788 集群上取证——该集群静态文件不是本 PR worktree，无法作为本 diff 的 Network 判据。

  **1.1 `call_result` 后调用既有 `unsubscribe()`（第 23 行）**：pass
  `git diff cfb6736..HEAD -- oamp/web/calls.js` 仅一行：
  ```text
  +    unsubscribe();
  ```
  位于 `handleCallEvent` 的 `if (type === 'call_result')` 渲染之后（`calls.js:163-167`）。无新函数、无新状态字段。`function unsubscribe` 仍为既有 `source.close(); source = null`。

  **1.2 关闭后 `selectedId` 保持；切行先关后开（第 24 行）**：pass
  `unsubscribe` 体不含 `selectedId`。独立 vm 探针：
  ```text
  after.selectedId 保持 task-aaa；header 仍含 task-aaa
  switched: firstClosed true, secondUrl /api/calls/task-bbb/stream
  ```
  浏览器独立页：终态后标题仍 `调用进度：task-live-1 completed`；再点第二行变为 `task-live-2`。

  **1.3 不依赖服务端新关流（第 25 行）**：pass
  独立 HTTP 在 `call_result` 后**保持 SSE 连接不 end**。浏览器仍关订阅：
  ```text
  afterFirst.state=completed logCount=1
  afterWait 1.6s 后 logCount 仍 1（retry:1000 若未关会再连并再收帧）
  STREAM_CONNS=2（两次选中各 1 次，等待期内无第三连）
  ```

  **1.4 真集群 Network 无每秒重连（第 26 行）**：fail
  本轮未对 :7788 既有集群加载本 PR 的 `calls.js`（集群静态面 ≠ 本 worktree）。PR 证据第 90–94 行自身记录：改动后真集群「没有向该 shell 调用发送 `call_result`」，Network 序列 `finish_count:0`。因此「收到 `call_result` 后连接关闭、无每秒重连」在真集群上**双方都未取到终态帧**。客户端机制由 1.3 的独立页覆盖，不替代本条字面「真集群」。

  **1.5 调用面板其余部分不变（第 27 行）**：pass
  `calls.html` 与 `cfb6736` 逐字相等。表头六列：调用 id / agent / 状态 / 开始时间 / 结束时间 / 模型。`ROSTER_REFRESH_MS = 5000`、`setInterval(loadCalls, ROSTER_REFRESH_MS)` 仍 1 处。`renderRoster` 仍 6 个 `<td>`。`app.js` 空 diff。

  **1.6 numstat 守门（第 28 行）**：pass
  ```text
  1	0	oamp/web/calls.js
  ```

  **1.7 零新依赖/样式/轮询（第 29 行）**：pass
  `package.json` 空 diff；`style.css` / `api-pages.css` 空 diff；`setInterval` 仍 1；`new EventSource` 仍 1；`<script>` 仍 1。

- **标准 2 · 改动面封闭性**：pass
  `cfb6736..HEAD` name-only：
  ```text
  docs/iterations/0029-hub-client-session-and-duplex/prs/pr-004-console-call-stream-stop-tasks.md
  docs/iterations/0029-hub-client-session-and-duplex/prs/pr-004-console-call-stream-stop.md
  oamp/web/calls.js
  ```
  ⊆ 文件范围 ∪ `*-tasks.md`（不构成夹带）。不触碰：`web.js` / `app.js` / 其余 `oamp/web/**` 零命中。

- **标准 3 · 验收证据原文照录**：partial
  通过：第 51–74、98–112 行 `$` + stdout（`git diff` / `numstat` / `node --check` / grep 计数）。
  不通过：第 78–94 行浏览器 JSON **无产生该输出的可复制命令**；第 84、94、122 行为转述（「该输出记录…」「客户端终态机制…替身注入」）。第 116–121 行自查命令命中自身。

- **标准 4 · 既有面零影响（S-2 / S-9 / Z-6）**：pass
  产品 diff 仅 `calls.js` +1 行。roster 6 列与 html 基线全等；静态白名单文件未改。S-9 要求的正是这一行适配。基线 `handleCallEvent` 在 `call_result` 后不 `close`（vm：`closed:false`）。

## 汇总

- pass: 2 项
- fail: 0 项
- partial: 2 项（标准 1 含第 26 行 fail 子项；标准 3）
- blocked: 0 项
...
| 规格/文档描述 | 实现实际行为 | 建议处理 |
|---|---|---|
| 验收 4 要求真集群 Network 无每秒重连 | 本 PR 静态文件不由 :7788 集群托管；证据段也未捕获 `call_result` | 开新迭代处理：用托管本 worktree 静态面的实例做 Network 取证，或把验收改为「客户端在服务端不关流时仍停订」 |
| 证据段混有 tab.run JSON 与转述 | 无 `$` 命令可复现浏览器块 | 按实现更新文档：附可复制的浏览器驱动命令 |

## 下一迭代候选

- 控制台 `onerror` 仍写「浏览器将自动重试」——终态 `unsubscribe` 后若服务端先断流，文案可能误导。
- F10 晚订阅补发落地后，需再验「重选已终态行仍能拿到补发且再次停订」。

## 结论

**PASS**（所有条目 pass 或 partial 且无 fail）

主 agent 可继续。第 26 行真集群 Network 未在本 worktree 静态面上取证，记在标准 1 partial 与偏差，不单独把委托级结论打成 FAIL。
