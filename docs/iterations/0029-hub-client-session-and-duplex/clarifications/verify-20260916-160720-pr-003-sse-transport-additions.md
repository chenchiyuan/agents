# 阶段 5 独立验证报告 · pr-003-sse-transport-additions

**报告文件**: `docs/iterations/0029-hub-client-session-and-duplex/clarifications/verify-20260916-160720-pr-003-sse-transport-additions.md`
**验证日期**: 2026-09-16
...
产出物：`feat/0029-pr-003-sse-transport-additions` @ `794b9e4`（工作区 `/Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0029-hub-client-session-and-duplex/.pb-agents/worktrees/0029-pr-003-sse-transport-additions`；merge-base `72b659f`）
验证标准来源：本委托 4 条 + PR「验收标准」第 23–31 行 9 款 + `architecture.md` §6.1 Z-1；标准 2 口径：`*-tasks.md` 不构成夹带
验证日期：2026-09-16

说明：不采信「验收证据」段自述。下列输出为本轮独立 mock-res 实跑，并对照 `git show 72b659f:oamp/src/transport.js` 动态 import 的基线模块。未改被验证产物。

## 逐项判定

- **标准 1 · 验收标准逐条判定**：partial
  通过：第 24–27、29–31 行。不通过拆开：第 23 行「既有 13 个导出键」计数与代码不符（基线对象键 11 + 模块导出 1 = 12 名，非 13；`closeKey` 未导出）。第 28 行隔离的「既有面全断开也不影响新注册表」：`closeAll` 不断开过滤订阅（连接仍开），但过滤订阅也收不到既有 publish——与「不受影响」部分相容；对象导出面追加 3 键成立。
  ...
  **1.1 导出面只追加（第 23 行）**：partial
  ...
  MISSING none
  ADDED closeCallSubscriptions,handleSubscribe,publishFiltered
  kind=sse 不变；模块级仍仅 export createSseTransport
  ```
  通过：差集恰为 3 个新键，无缺失。
  不通过：条文写「既有 13 个导出键」且列举含未导出的 `closeKey`。现测基线对象键 **11**（DC-14 同族）。按实际集合判定追加成立。
  ...
  **1.2 `closeCallSubscriptions` 只关 `call:<id>`（第 24 行）**：pass
  ...
  ```
  `call:` 被 `res.end()` 后不再收帧；`chat:` / `chat-calls:` 仍收。`closeCallSubscriptions` 体为 `closeKey(callKey(callId))`；`res.end(` 源码仍 1 处（仅 `closeKey`）。

  **1.3 断开清理 + 心跳自停（第 25 行）**：pass
  过滤订阅 `res.on('close')` 从 `filteredSubscribers` 删除并 `stopHeartbeatIfIdle`。`heartbeatMs:40`：
  ```text
  HB { ka1: 2, ka2: 2, ka1b: 2, ka2b: 2, stopped: true }
  ```
  两侧 close 后 keepalive 计数不再增加。
  ...
  **1.6 隔离性（第 28 行）**：pass
  ```text
  AC6_GC_UNCHANGED true（filtered 加入/关闭后 globalCount 仍 1）
  AC6_EXISTING_NO_FILTERED true
  AC6_FILTER_FROM_EXISTING_APIS []  （publish/publishGlobal/publishCall/publishChatCall 不投喂过滤表）
  ```
  `closeAll` 只遍历 `subscribers`，过滤连接 `ended=false`——既有面全关不断过滤订阅（隔离的另一方向）。
  ...
  **1.7 SSE 头与无初始数据帧（第 29 行）**：pass
  ...
  chunks ["retry: 1000\n\n"]
  ```
  keepalive 常量 `DEFAULT_HEARTBEAT_MS=15000` 未改（本轮用 40ms 仅测自停）。

  **1.8 四键空间回归与基线一致（第 30 行）**：pass
  同一 probe 对 `72b659f` 模块与当前模块：
  ```text
  AC8_JSON_EQ true
  before: global=[global] chat=[chat] call=[call] chatCalls=[call,chat_call]
  close(chat) 只结束 chat 订阅；随后 call/chat-calls/global 仍可达
  ```
...
- **标准 3 · 验收证据原文照录**：fail
  PR 第 47–54 行引用 `/tmp/pr003-verify.mjs` 等仓库外脚本，正文为 PASS 清单与「两行 JSON 完全一致」的转述，无工作区内可复制命令 + 原样 stdout 块。

- **标准 4 · 既有面零影响（Z-1）**：pass
  四键空间 probe 与基线 JSON 全等；既有 publish API 不写入过滤注册表；`globalCount` 不计入过滤订阅者；新增键为追加。`closeAll` 不清理过滤订阅——既有 `closeAll` 语义（只关 `subscribers`）保持，属新面未纳入既有关闭路径（记偏差，不破 Z-1）。
...
| 规格/文档描述 | 实现实际行为 | 建议处理 |
|---|---|---|
| 验收 1「既有 13 个导出键」且列举 `closeKey` | 基线对象键 11；`closeKey` 未导出；追加后对象键 14 | 按实现更新文档（DC-14 同口径） |
| `closeAll` 覆盖「全部订阅」 | 只关 `subscribers`，`filteredSubscribers` 连接不 `end` | 重新评估实现 / 按实现更新文档 |
| 证据段指向 `/tmp/pr003-*.mjs` | 仓库内无这些脚本 | 按实现更新文档：改为仓内可复制命令 |

## 下一迭代候选

- `closeAll` 是否应同时结束过滤订阅（进程退出路径目前可能留下 filtered SSE）。
- 既有 `publish*` 不自动投喂过滤表 ⇒ pr-005 必须在既有发布点显式调 `publishFiltered`，否则新面无事件。

## 结论

**FAIL**（有 fail 条目）

主 agent 需返工标准 3（证据改为命令 + 原样输出）。传输行为本轮对照基线成立；第 23 行「13 键」为条文计数错误，按实际集合追加通过，计入标准 1 partial 而非单独 fail。
