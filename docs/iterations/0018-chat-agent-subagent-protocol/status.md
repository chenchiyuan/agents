# 工作流进度

**工作流**: workflow-pb **v0.10.0**（自本迭代恢复起适用；阶段 1~2 在 v0.8.0 下完成；v0.9.0 的 cwd 启动门已由 v0.10.0 取代）
**迭代**: 0018-chat-agent-subagent-protocol
**当前阶段**: 阶段 6（独立验证，**已完成 PASS**；迭代已完成，待合并 main）
**迭代分支**: iteration/0018-chat-agent-subagent-protocol（**已合并进 main `1820f8c`，分支已删除**）
**基线**: 已并入当前 main（含迭代 0017 全部产出 + 0019 的 v0.9.0 + **0020 的 v0.10.0 / SKILL v1.13.0 / 协议产物 `docs/worktrees/README.md`**）
**状态**: **已完成并合并**（阶段 1~6 全部完成；阶段 6 PASS 13/13、逐卡 16/16、E1~E11 11/11；迭代分支已合并 main 并删除；会话工作区已清理）
**history**: 开启
**前置**: 迭代 0019 已完成并合并 main（workflow-pb v0.9.0 / SKILL v1.12.0 生效）；用户裁决 ① = 按新规范为 0018 建自己的会话工作区后继续阶段 3。

## 恢复设置（2026-09-12，迭代 0019 完成后）

| 项 | 值 | 依据 |
|---|---|---|
| 会话工作区 | `/Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0018-chat-agent-subagent-protocol` | workflow-pb v0.10.0 规则 C（落点 = `<仓库主工作区>/.pb-agents/worktrees/{迭代ID}`） |
| 检出分支 | `iteration/0018-chat-agent-subagent-protocol`（tip 随阶段 5 推进前进；恢复时为 `deeb1b7`） | 规则 C（阶段 1~6 期间保持在该分支） |
| 基线整合 | 已把当前 main（含 0017 全部产出 + 0019 的 v0.9.0 / v1.12.0）合并进本迭代分支；0017 文档冲突以 **main 版为准**（0017 的权威态），本迭代自身产物零丢失 | 规则 A/B + 结构正确性 |
| 启动判据 | **v0.10.0 起改为「地址解析 + 寻址正确性」**（原 cwd 三项判据已随规则 D 改写删除）；实测：解析链得出的工作区地址 = 上表「会话工作区」行、`git -C <该地址> branch --show-current` = `iteration/0018-chat-agent-subagent-protocol` | 规则 C / 规则 D（v0.10.0） |
| 角色定义来源 | `roles/workflow-pb/workflow-pb.md` 等**随检出即得**（被 git 追踪）；`.pb-agents/roles/` 在新工作区**不存在**（已由 v0.9.0 降级为面向下游的可选分发手段） | 规则 §角色定义来源与部署（W8） |
| 生效协议版本 | **workflow-pb v0.10.0**（阶段 3~6 按此执行：任意位置启动 + 显式寻址 + PR worktree 为会话工作区子层 + 跨工作区写入禁止） | 迭代 0020 交付物；规则 C/D/H |
| 下一步 | 阶段 5 收尾（pr-005 合并）→ 阶段 6（独立验证）；输入 = `prs/` 五份 PR 文件 + `architecture.md` | workflow-pb 阶段定义 |

> **启动形态（v0.10.0 规则 C/D/H）**：会话**可在任意位置启动**（含仓库主工作区、任意子目录）；须**解析并声明本迭代工作区地址**（见下表），此后一切文件写入与 git 写操作**显式寻址该地址**（绝对路径 / `git -C <地址>`）。**不再有 cwd 判据、不再要求 cd、不再有启动门**；会话工作区不存在时由会话自建；唯一硬停面 = 「工作区无法确定或无法创建」。

## 阶段状态

| # | 阶段 | 完成 | 已验证 | 备注 |
|---|---|---|---|---|
| 1 | 需求收敛 | ✅ | ✅ | demand.md v1.2.0 已收敛；P-1~P-11 与 Q-1 全部经用户逐条裁决；`model_inferred` 归零 |
| 2 | 功能规格 | ✅ | ✅ | prd.md v0.2.0 已收敛；16 张卡（F01~F13 + 保证项 F14~F16）；MI-01~MI-08 经用户裁决「全部按推荐」并落卡；卡内 `[model_inferred]` 零残留（T-01~T-17 已于阶段 3 补齐） |
| 3 | 技术架构 | ✅ | ✅ | `architecture.md` v1.1.4（686 行）+ 16 卡 `[架构待填]` 全填；T-01~T-17 = 17/17；L1 = 0 条；自查 14 项全 ✅ |
| 4 | PR 规划 | ✅ | ✅ | 阶段 4→5 的 Gate 验证报告：`clarifications/verify-stage4-gate-20260912-231000.md`（PASS 9/9） |
| 5 | PR 实现 | ✅ | ✅ | 五个 PR 全部合并进迭代分支；「已验证」= 含 PR 级验证（7 份 PR 级报告 + 2 轮复验）（`b92ad82`/`95051b0`/`7e351fa`/`3a8f0d3`/`bb1e26b`）；pr-003 与 pr-005 各经一轮修复后复验 PASS |
| 6 | 独立验证 | ✅ | ✅ | 报告 `clarifications/verify-stage6-20260913-012611.md`：**标准 13/13 pass、逐卡 F01~F16 16/16 pass、E1~E11 11/11**；阶段 5 的「并发调度真实执行证据三项」全 pass；端到端真起服务实跑通过；偏差 14 条（均不阻塞）、下一迭代候选 8 条。`clarifications/` 共 8 份验证报告 = **PR 级 7 份 + 阶段 4 Gate 1 份** |

## 并发配置（阶段 5）

| 字段 | 值 |
|---|---|
| **起始并发数** | 3 |
| **硬上限** | 5（公式 `2×起始-1`） |
| **当前有效上限** | 5（= min(3 + 5×3, 5) → 钳到硬上限；五次槛位释放后维持 5） |
| **累计槛位释放次数** | 5（pr-001~pr-005 各 +1；按规范「失败/阻塞判定同样释放槛位」口径，pr-003 与 pr-005 的验收 FAIL 亦计入其自身释放，未额外增加）。口径说明：**首波 pr-001/pr-002 同批合并，「释放 = 1」未单独记录**（一次记为 2）；按公式 `min(3+3,5)=5` 结果不受影响 |
| **已派发总数** | 5（pr-001~pr-005 全部已派发） |

依赖图（阶段 4 产出）：`{pr-001, pr-002} → pr-003 → {pr-004, pr-005}`；首波解锁 = {pr-001, pr-002}。

## PR 实现子状态（阶段 5 展开）

| PR 文件 | depends_on | 状态 | worktree 分支 | 已合并 | 槛位状态 |
|---|---|---|---|---|---|
| pr-001-transport-call-key-namespace.md | （无） | ✅ 已合并 | `feat/0018-pr-001-transport-call-key-namespace`（已删除） | `b92ad82` | 已释放 |
| pr-002-registry-task-list-model.md | （无） | ✅ 已合并 | `feat/0018-pr-002-registry-task-list-model`（已删除） | `95051b0` | 已释放 |
| pr-003-call-http-surface-and-contract-docs.md | pr-001, pr-002 | ✅ 已合并（验收 FAIL→修复→复验 PASS 28/28） | `feat/0018-pr-003-call-http-surface-and-contract-docs`（已删除） | `7e351fa` | 已释放 |
| pr-004-console-call-page.md | pr-003 | ✅ 已合并（验收 PASS 14/14） | `feat/0018-pr-004-console-call-page`（已删除） | `3a8f0d3` | 已释放 |
| pr-005-call-protocol-acceptance-tests.md | pr-003 | ✅ 已合并（验收 PASS 30/30 → 定向修复 → 复验 PASS 20/20） | `feat/0018-pr-005-call-protocol-acceptance-tests`（已删除） | `bb1e26b` | 已释放 |

## 待确认项

- [ ] **C-1（工作区共享冲突，已登记；用户已知晓）**：外部会话在同一工作区完成迭代 0017 收口（阶段 6 PASS → 合并 main `03a2f00` → 删除 0017 分支），其收口提交 `028ca8a` 落在本迭代分支上且**不在 main**。主 agent 的处置：不改写任何 ref、不删除他人提交、不替 0017 做提交决策。用户裁决（本轮）：这正是需要 0019 解决的根因。
- [ ] **C-2（0017 两处残留的最终去向；2026-09-12 18:20 按 progress-observer 独立核实修正）**：① `docs/iterations/0017-project-workspace/history.md`（外部会话 17:42:52 收口记录）；② `docs/iterations/0017-project-workspace/clarifications/verify-stage6-20260912-173934.md`（工作区版本曾比 `028ca8a` 中版本第 160 行多一处 `:695-699` → `:695-698` 修正）。**实际情况（修正原表述）**：主 agent 执行 `git stash push` 时已无内容可保存——外部会话已抢先把两处残留提交进 main（`9ede9ea`，17:46:02）；实测 `git stash list` 为空、`refs/stash` 不存在，**恢复 0018 时无需 stash pop**。工作区版本的 `:695-698` 修正仅存于 `/tmp/pb-0017-residual/`（3 文件备份，mtime 17:42~17:44）。0017 收尾产物以 main 中的版本为准；其提交版内部的一字符自相矛盾见 0019 台账 D-4 留痕。
- [ ] **C-3（0017 遗留缺陷 D-2，用户裁决：不纳入本迭代）**：`/api/chats`、`/api/messages` 路由登记元数据缺 `project_id` —— 不在本迭代完成。**归属（2026-09-12 18:20 更新）**：用户裁决由 **0019 完成后起的单独小迭代承接**，三处 status（0017 / 0018 / 0019）互相引用留痕；0018 恢复时不承接该项（除非用户另行指示）。
- **无其他用户待确认项**（P-1~P-11、Q-1、MI-01~MI-08 均已裁决并落盘）。

## 用户确认记录（Gate）

- **阶段 0**：迭代 ID = `0018-chat-agent-subagent-protocol`；0017 前置处理 = 「0018 从 0017 迭代分支起」。
- **阶段 1 第二轮（P-1~P-11）**：P-1 = A（参照 harness `task` 工具契约）；P-2 = A 并追加身份规则（用户原话：「同hub和agent之前的约定， 同chat下，同名agent是一个agent，共享上下文。 不同chat下agent是不同agent」）；P-4 = A（`task_id` 升格为 HTTP 一等调用资源，解除 0015 N5 但限「按调用 id 读自己发起的调用」）；P-3/P-5/P-6/P-7/P-8/P-9/P-10 = 全部按推荐；P-11 = 「都没想过，交给架构阶段」。
- **阶段 1 第三轮（Q-1）**：= ①（`one_shot` 保留为显式例外，登记差异 ⑲）。
- **阶段 2（MI-01~MI-08）**：全部按推荐采纳（八项验收口径定案）。
- **范围项（C-3）**：D-2 不纳入本迭代。
- **流程项（新需求）**：并行会话各用独立 worktree/clone、不得共用同一工作树 → 迭代优化 workflow-pb，优先级最高 → 落地方式 = 「先做 0019（独立迭代），暂停 0018」。

## 更新日志

- 2026-09-12: 迭代启动（Step 0 前提声明：本宿主具备真实阻塞式人机交互通道 `ask`）。
- 2026-09-12: 阶段 0 用户裁决两条（迭代 ID、0017 基线处理）→ 派发 demand（第一轮）。
- 2026-09-12: 阶段 1 三轮收敛：v1.0.0（待确认版）→ P-1~P-11 用户裁决 → v1.1.0 → Q-1 用户裁决 ① → v1.2.0（`model_inferred` 归零）→ 阶段 1 推进条件通过 ✅。
- 2026-09-12: 创建迭代分支 `iteration/0018-chat-agent-subagent-protocol`（base = `iteration/0017-project-workspace` @ 428e672）→ 阶段 2 派发 prd + 自动派发 progress-observer。
- 2026-09-12: progress-observer 独立核实发现外部会话在同一工作区完成 0017 收口（`028ca8a` 落在本分支、main 得 `03a2f00`、0017 分支已删）→ 主 agent 以零丢失方式处置（备份 → `checkout -m` → 并集解冲突 → 恢复残留）并切回本分支。
- 2026-09-12: 阶段 2 两轮：prd v0.1.0（16 张卡 + 8 项 MI 待裁决）→ 用户裁决「全部按推荐」+ D-2 不纳入 → prd v0.2.0 收敛（`model_inferred` 零残留）→ 阶段 2 推进条件三项通过 ✅。
- 2026-09-12: 用户提出新需求（并行会话工作区隔离，优先级最高）并裁决「先做 0019、暂停 0018」→ 本迭代置**已暂停**；阶段 1~2 产物提交进 `iteration/0018-chat-agent-subagent-protocol`；0017 两处残留具名 stash 保存；工作区切至 0019。

- 2026-09-12: 按 v0.10.0 恢复——整合新 main（`67c50e0`，含 0017 + 0019 + 0020）；§恢复设置 更新为「任意位置启动 + 显式寻址」形态（旧 cwd 前置段作废）；下一步 = 阶段 3（技术架构）。

- 2026-09-12: 阶段 3（技术架构）完成并推进：`architecture.md` 686 行 + 16 卡 `[架构待填]` 全填；**L1 = 0 条**；§11 必然变更点 10 条；§10 非 L1 口径点 8 条；自查 14 项全 ✅ ⇒ 进入阶段 4（PR 规划，pr-planner 已派发）。

- 2026-09-12: 阶段 4 Gate **PASS 9/9**（`clarifications/verify-stage4-gate-20260912-231000.md`，4 条非阻塞偏差）⇒ 进入阶段 5；并发配置初始化（起始 3 / 硬上限 5 / 有效 3 / 释放 0 / 已派发 0）。

- 2026-09-12: 首波 {pr-001, pr-002} 并发实现并验收（pr-001 PASS 32 项 / pr-002 PASS 19 项）→ 合并 `b92ad82`/`95051b0` → 槛位释放 2（有效上限 3→5=硬上限）；下游 pr-003 解锁。

- 2026-09-13: pr-003 验收 FAIL（终态漂移 A20 + `exit_code` 文档 D2）→ 修复轮两轮（`f771ae5`/`baf9c43`）→ **复验 PASS 28/28** → 合并 `7e351fa`；槛位释放 3（有效上限钳至硬上限 5）；**pr-004 与 pr-005 同时解锁**（末波并发）。

- 2026-09-13: pr-004 验收 **PASS 14/14**（含浏览器实测）→ 合并进迭代分支；槛位释放 4（有效上限维持硬上限 5）。

- 2026-09-13: pr-005 复验 **PASS 20/20**（verifier 另做 4 组变异实验证明新断言具鉴别力）→ 合并 `bb1e26b`；**阶段 5 完成**（五 PR 全部合并）；进入阶段 6。

- 2026-09-13: **阶段 6 PASS**（13/13 标准 + 逐卡 16/16 + E1~E11 11/11 + 并发三项全 pass + 端到端实跑），迭代完成，进入收口合并。

- 2026-09-13: **收口完成**——迭代分支合并进 main（`1820f8c`，无冲突）；删除迭代分支与会话工作区；交付面冒烟见 main 上验证记录。

- 2026-09-13: 按阶段 6 报告的偏差 D-3/D-4/D-5 订正台账口径（验证报告计数 = PR 级 7 + Gate 1；阶段 5「已验证」列口径 = 含 PR 级；槛位「释放 = 1」未单独记录的说明）。
