...
- `oamp/sdk/surface.js`（**修改**：`runApi` 按 **`mode` 分支**派生 `headerTimeoutMs` 注入 `requestSpec`：`mode === 'block'` 时按 **`(waitMs ?? DEFAULT_WAIT_MS)`** 派生（含 1s 余量；`DEFAULT_WAIT_MS` = `oamp/sdk/cli.js:18` 的 `1800000`，用于库面未给 `waitMs` 的兜底），其余 mode（含 `background`、含省略 `mode`）**不下发该参数**（`http.js` 侧取 5000ms 缺省）；派生后的预算须保证响应头上限**不得先于** `--wait` 上限触发（`oamp/sdk/http.js:141-151` 按 `waited` 归类）。`ENTRIES` 40 条的字段集与条目数不变、不新增条目属性 —— `mode` 是 `oamp/sdk/surface.js:166` 已声明的 `str('mode')`，读取它不增删条目字段）
- `oamp/test/sdk-cli-contract.test.js`（**修改**：① 把 `T4 [已知偏差 · 归 pr-011 收口修复]` 翻转为规格期望 `WAIT_TIMEOUT`/`1`、`error` 含 `8000`、耗时窗移到 `--wait` 上限附近，并删掉用例内「本 PR 不改 `oamp/sdk/**`」的注释与偏差锁用例名；② `T7` 同文件**三处**「零判别力非空前置」一并由 `length > 0` 改为 `trim()` 后判非空：两个只读组（`:1072` `status`、`:1077` `task list`）**外加经主 agent 裁决一并纳入的第三处** —— `:1086`（修复后现 `:1185`）的 `unreachable.viaHub.stderr` 非空前置（同一缺陷类、同文件、一行改动））
...
> 逐文件说明：五个文件**全部是修改既有文件**，无新建。前两个（`http.js` / `surface.js`）是一处修复的两个半边，必须同批落地（只改一个 ⇒ 要么上限仍先行、要么参数无人派生）。后三个各含一类独立修复，彼此无引用边。`sdk-cli-contract.test.js` 被两类修复共同触及（偏差锁翻转 + 三处非空前置），须由同一执行者串行处理（同文件多处编辑，不存在跨任务合并）。
...
- [ ] **A-上限内成功（>5000ms 段）**：服务端 **6000ms 后**正常响应、**显式**给 `--mode block` 与 `--wait 8000` ⇒ 退出码 `0` + 响应体原样（`{"calls":[…]}` 信封不加壳、不改字段名）；**两宿主覆盖面**：库面 `createHub().api.calls.create({ …, mode: 'block', waitMs: 8000 })` 对同一 6000ms 桩同样 ⇒ 返回该响应体原对象（不加壳）、耗时同量级。耗时落在 `(6000, 7000)`ms (F10 验收 2 —— 该形态当前实测 `5042ms → 3/REQUEST_TIMEOUT`)
- [ ] **A-缺省上限可达（有界观测，不实测 30 分钟）**：服务端 6000ms 后响应、**显式**给 `--mode block` 且**不给** `--wait` ⇒ 退出码 `0` + 响应体原样；耗时 ~6000ms 量级（当前实测 `5041ms → 3/REQUEST_TIMEOUT`）。缺省 `1800000` ms 的「可达」由「6000ms > 5000ms 仍走等待上限」+ `oamp/sdk/cli.js:18` 的 `DEFAULT_WAIT_MS` 常量面共同闭合。**两宿主覆盖面**：库面 `createHub().api.calls.create({ …, mode: 'block' })`（**不传 `waitMs`**）对同一 6000ms 桩同样 ⇒ 退出码 `0` + 响应体原样、耗时 ~6000ms 量级（**不落回 5000ms**）—— 与 CLI 面「不给 `--wait`」同判据、同观测 (F10 验收 1)
- [ ] **A-小于 5000ms 的原有路径不回归**：`--wait 1000` / `600` / `1500` 对黑障桩仍 ⇒ `WAIT_TIMEOUT`/`1`，`error` 含对应上限值，耗时随上限单调（`600` 先于 `1500` 返回），每次恰 1 次请求
- [ ] **A-非 block 形态上限不变（5000ms 缺省未被放大）**：① 不带 `wait` 声明的层 A 条目（如 `hub api docs`）对同一黑障桩仍 ⇒ `REQUEST_TIMEOUT` / 退出码 `3`、耗时 ~5000ms、`error` 含 `5000`；② `api calls create` 的**非 block 形态**（`--mode background`、以及**省略** `mode`）对同一黑障桩同样 ⇒ `REQUEST_TIMEOUT` / 退出码 `3`、耗时 ~5000ms、`error` 含 `5000` —— **即使给了大值 `--wait 8000`** 也不把响应头上限放大到 `8000`/`1800000`（`mode` 分支口径的判据面；见「A-单点派生」条）(F07 验收 1~5、`architecture.md` §5.4)
...
- [ ] **A-单点派生、两宿主同享**：`waitMs → headerTimeoutMs` 的派生只落在 `oamp/sdk/surface.js` 的 `runApi` 一处（全仓 `headerTimeoutMs` 的**派生点**恰 1 —— 含下述 `mode` 分支在内，全部落在 `runApi`；`oamp/sdk/http.js` 只负责「取缺省值」）⇒ 库面 `createHub().api.calls.create({ …, mode: 'block', waitMs: 8000 })` 与 CLI 面对同一黑障桩得到同一 `code` / `exitCode`。SDK 不因此新增第二条上限判定；派生**按 `mode` 分支**：`mode === 'block'` 时响应头上限 = **`(waitMs ?? DEFAULT_WAIT_MS) + 1s 余量`**（`DEFAULT_WAIT_MS` = `oamp/sdk/cli.js:18` 的 `1800000`；余量用以保证 `--wait` 上限必先到）——**库面未给 `waitMs` 时按 `DEFAULT_WAIT_MS` 派生**（CLI 面的等价兜底在 `oamp/sdk/cli.js:255`；库面若无此兜底则两个消费面不等价 ⇒ 撞 F01「同一份代码两种消费方式」），**其余 mode（含 `background`、含省略 `mode`）保持 5000ms，不随 `--wait` 缺省值 `1800000` 放大**（依据 `architecture.md` §5.4 与 `prd/F10` 卡原文「可阻塞条目恒有上限（默认 `1800000`）；非可阻塞条目走 5000ms」）。（原句「不按 `mode` 取值做语义分支」**已于主 agent 裁决后取代** —— 裁决理由：缺省 `1800000` 只应服务真正的阻塞调用；`mode` 是 `oamp/sdk/surface.js:166` 已声明的 `str('mode')`，读取它不增删入口表字段，故不撞 `ENTRY_FIELDS`）(F02 验收 2 的三宿主等价物、W3 / N5 / F11 验收 5)
...
- [ ] **B3-载荷丢失可捕获**：`oamp/test/sdk-cli-contract.test.js` 的 T7 三处非空前置（两个只读组 + `unreachable.viaHub.stderr`，第三处经主 agent 裁决纳入）改为 `trim()` 后判非空 ⇒ `"\n"` 不再满足该前置。**判别实验**：副本内把 `oamp/src/task.js` 的 `renderList` 空态 `'（无任务）'` 改为 `''`（stdout 变 `"\n"`，两侧仍逐字节相等）⇒ 该用例必红（当前 `stdout.length > 0` 下为绿）
