...
- `roles/workflow-pb/data/workflow-pb-changelog.md`（**追加**）：头部新增 `## v0.10.0（2026-09-12）` 条目——**段构成（段序 / 段数 / 各段内容）一律以 `architecture.md` v1.1.3 §8.3 的 changelog 段构成裁定为唯一真源，本 PR 不复述**；第 6 段「具体改动」需逐条覆盖 `pr-001` 的规范侧与 SKILL 侧改动（含版本号迁移句）；第 7 段「备案」内容同 §9.1 / §9.2 与 F16；既有 v0.2.0~v0.9.0 条目**逐字不动**
...
- `.claude/skills/workflow-pb/data/skill-optimization-v1.13.0.md`（**新建**）：**段名与段数一律以 `architecture.md` v1.1.3 §8.3 的落点表为准（本 PR 不复述）**；体例沿用 `data/skill-optimization-v1.12.0.md` 的先例精神（粗体段名 + 段间空行 + 逐条可核对 + `:1` 版本对声明行；该先例的实际段构成见其文件自身，不在本 PR 复述）；「更正登记」段为本记录新增段，需写明 `:13` / `:40` 两处改成了什么
...
- [ ] **changelog 条目（F11 验收 1~2 / F15 验收 1~4 / F16 验收 2）**：`roles/workflow-pb/data/workflow-pb-changelog.md` 中含 `## v0.10.0（2026-09-12）` 条目——**段构成（段序 / 段数）一律以 `architecture.md` v1.1.3 §8.3 的 changelog 段构成裁定为唯一真源，本 PR 不复述**；判据 = ① 段序与 §8.3 一致；② 「被取代清单」逐句引 v0.9.0 原句（章节 + 原句，不用行号，同 §8.3-T-07）、「保留清单」覆盖 §7.2 的六组、「未改变的部分」覆盖 §3.5 的零改动面、第 6 段「具体改动」逐条对应 `pr-001` 的 `N-01`~`N-21` 与 `K-01`~`K-09`；③ 第 7 段「备案」内容同 §9.1 / §9.2 与 F16
- [ ] **备案段镜像（F15 验收 3 / F16 验收 2）**：changelog 条目的**第 7 段「备案」**与规范 `## v0.10.0 变更说明` 的备案段（规范侧为**第六段**）**同源**——① 0019 E 项三分类（保留 E1~E4 / E6 / E7 / E9~E13、改写 E5 与 E8、**明确登记为两条新增**）；② `0018/status.md` 的「操作前置（宿主限制，必须由用户执行）」句已失效的登记，且**不含**"0018 需回改 / 本迭代将修改 0018 文件"类表述（F16 验收 5），`docs/iterations/0018-*/**` 在本 PR 的改动清单中 0 命中（F16 验收 1）
...
- [ ] **SKILL 新记录（F15 / F18 验收 3）**：新建 `.claude/skills/workflow-pb/data/skill-optimization-v1.13.0.md`——**段名与段数一律以 `architecture.md` v1.1.3 §8.3 的落点表为准（本 PR 不复述）**；判据 = ① 该文件 `:1` 的版本对声明与 `SKILL.md` 的版本行一致；② 「更正登记」段可追溯到"改了哪两处（`:13` / `:40`）、改成什么"，且与实际落地文本一致（其落地由 `pr-004` 承担，`depends_on` 已声明）
...
- `docs/iterations/0020-session-workspace-addressing/architecture.md` **v1.1.3**（**本 PR 的直接验收基准**；v1.1.3 未落盘时以 v1.1.2 / v1.1.1 为准）：§3.4（`O-01` / `O-02` / `O-04`）、§3.5 零改动清单、§3.6 版本一致性判据、§8.3 的「其余落点」表与 changelog 段构成裁定、§9.1 / §9.2。**段序 / 段数一律以 architecture v1.1.3 §8.3 为准，本 PR 不复述**
