...
**版本**：1.1.4（阶段 3 产物；**L1 三条已由用户确认并落盘**；**Gate r1 / r2 / r3 偏差均已闭合——残留双写法全文件清零**）
**版本轨迹**：v1.0.0（初版：L1 三条待确认、L1-4 分级待复核）→ **v1.1.0**（2026-09-12：L1-1 / L1-2 / L1-3 转「已确认 = 按推荐」并逐条附用户裁决原文；L1-4 由主 agent 裁定维持 L2；三条落定的连带同步见 §6.2 末段与 §12）→ **v1.1.1**（2026-09-12：**触发来源 = Gate 阶段 4→5 入口独立验证上报的 10 条偏差中属本文件的两条**——D-4（`§3.1-N-18` 与 `§8.1-M` 对「第 4.5 步字段写入职责」表述不一致）、D-10（协议产物节的位置约束在本文件内存在「紧邻规则 H」与「规则 H 之后、约束生效范围之前」两种措辞）；**裁定者 = 主 agent（L2）**；落点 = §3.1-N-06/N-13/N-18 + §3.1 新增「新增三节的位置口径」段 + §6.1-L2-6 + §8.1-J / §8.1-M 重写 + §12 两行 + §13 写入范围）→ **v1.1.2**（2026-09-12：**触发来源 = Gate r2 报告（`clarifications/verify-gate-stage4-r2-20260912-212007.md`）的两项 partial 与 8 条偏差中属本文件的三项**——项 1 = `v0.10.0` changelog 条目的段构成裁定（同源偏差 Gate r2 D-1 / D-2 / D-5）⇒ 定为 **7 段**（第 6 段固定「具体改动」、备案为第 7 段新增）；项 2 = Gate r2 **D-3**（`§8.2-K-09` 与 `§3.2-K-09` 对同一「三处版本引用」的行号写法不一致，已按实测统一）；项 3 = Gate r2 **D-4**（`O-02` 的 `memory.md`「首行插入」与实测「既有最新版本行之上」不一致）；**裁定者 = 主 agent（L2）**；落点 = §3.4-`O-01`/`O-02` + §3.2-`K-09` + §8.2-`K-09` + §8.3 两表与新增「段序依据」行 + §12 两行 + §13 写入范围）→ **v1.1.3**（2026-09-12：**触发来源 = 主 agent 对 v1.1.2「规范侧与 changelog 条目共用同一段序」表述的裁定**——两处**段序独立**（规范侧 `## v0.10.0 变更说明` = **6 段**、changelog 条目 = **7 段**），**唯一共同点 = 备案段内容同源**；**裁定者 = 主 agent（L2）**；落点 = §8.3 分列的表 A / 表 B 与两行「段序依据」+ §3.4-`O-01` + §12 一行 + §13 写入范围）→ **v1.1.4**（2026-09-12：**触发来源 = Gate r3 报告的 1 项 partial（措辞级）——本文件残留的「同一事实两套写法」**（`§2.7` 行 9 的版本引用旧行号与 `§8.2-K-09` 的实测口径并存）；**裁定者 = 主 agent（L2）**；落点 = §2.3 行 4 + §2.7 行 9 + §1.1 两行 + §3.1-`N-08` / `N-13` + §3.4-`O-01` + §3.5 两处范围 + §7.2-`P6` + §12 段序行与新增「Gate r3」扫描行 + §13 写入范围；**被替换的旧值只在 §12 的「Gate r2 项 1 / 项 2 / 项 3 已闭合」行出现一次**（作"已更正的历史叙述"），全文其余处不复现）
**日期**：2026-09-12
**状态**：阶段 3 **已完成**——L1 三条经用户确认（逐条裁决原文见 §6.2）、L1-4 维持 L2、T-01~T-16 全填（§11）、21 张卡架构段完整、内部一致性自查 **27 项**逐条通过（§12，含 12 项冲突核对 + Gate r1 / r2 / r3 偏差、段序裁定与四类事实的单一权威扫描）。**阶段 4（PR 规划）可据此启动**；§1~§5 与 §7~§11 为**硬契约**（变更面、条款落点与措辞、取代与保留清单），阶段 4 拆 PR 与阶段 5 实现不得偏离。
...
graph TB
  ...
  SPEC["roles/workflow-pb/workflow-pb.md ★<br/>规则 C 改写 · 规则 D 更名「启动与寻址契约」· 新增规则 H · 新增「协议产物」· 新增「约束主体」<br/>隔离边界声明补写 + 章节级判断方式 · 文档路径协议基准句 · brief 字段"]
...
| 4 | `workflow-pb.md:33/35`（CRITICAL） | 「本文件是提交管理约束的唯一来源」（`:35`）与「修改本文件不得侵入任何执行角色文件」（`:33`） | **保留**（元约束） |
...
| 9 | `SKILL.md:4` / `:30` / `:60`（行号以实测为准，Gate r3 偏差） | `description`（v0.9.0）/ `**版本**: 1.12.0（对应规范 workflow-pb v0.9.0）` / Purpose「按 workflow-pb v0.9.0 规范调度」 | **改写**（三处版本同步 → v0.10.0 / 1.13.0；同 0019 ADR-2 判据：改后 `grep -n 'v0\.9\.0' SKILL.md` = **0 命中**；**行号已按实测更正**，Gate r3） |
...
| **N-04** | `### 规则 D`（`:165-184`，标题 + 正文 + 硬停块 + 判断方式） | 改写（标题更名 + 全文重写；**L1-1 ② 已确认**） | 更名 `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）`（**保留编号 D**；用户裁决用语为「启动与寻址契约」，本行以之为生效标题）；正文 = 解析链（主工作区求取 → 地址 → 显式覆盖）/ 建立（缺省时，由会话执行，命令形态不变）/ 建立成功依据 / **唯一硬停面（工作区无法确定或无法创建）** + 硬停输出形态（三行事实陈述，见 §5.4）/ 判断方式（地址锚定 + 事后可复核）；**取代** `:167` 首句与 `:169-171` 三条判据作为启动门、`:173-181` 硬停块 | F09 验收 1/4/5/6、F01 验收 3、F06 验收 1~4、F12 验收 5（不承诺阻止）、F20 | F01 / F06 / F09 / F12 |
| **N-05** | 新增节，插入点 = `### 规则 G` 末行（`:206`）之后、既有 `### 隔离边界声明`（`:208`）之前（位置口径见本节表后「新增三节的位置口径」） | 新增 | `### 规则 H：显式寻址纪律`（v0.10.0 新增）：一切文件写入以**声明地址下的绝对路径**进行；一切 git **写**操作用 `git -C <地址>`；不依赖 cwd、不依赖任何工具；**不得表述为"越界会被阻止"** + `判断方式：` 行（执行者自检 + 事后核查引用） | W2 / F02 验收 1~4 / F20 验收 1、4；体例沿用既有「规则 X + 判断方式」 | F02 / F20 |
| **N-06** | 新增节，**紧邻 `### 规则 H` 之后**（= 位置口径的生效表述；三节次序见本节表后「新增三节的位置口径」） | 新增 | `### 协议产物：工作区间与生效规范（`docs/worktrees/README.md`）`（v0.10.0 新增）：产物落点与被跟踪（随检出即得）/ 必须覆盖的 5 类信息（模板）/ **非第二真源**（实例化投影，一切规范性陈述以引用形态出现）/ **落点语义区分句真源（T-15）** / 不含脚本·钩子·守卫 / `判断方式：` 行 | W3~W7 / F03 / F04 / F05 验收 1~2 / F21 验收 5；N11（真源一处） | F03 / F04 / F05 / F21 |
| **N-07** | 新增节，紧邻 `### 协议产物` 之后（次序第三条，见本节表后「新增三节的位置口径」） | 新增 | `### 约束主体`（v0.10.0 新增）：**对人**段（无目录要求 / 无前置命令 / 无 cd / 无重开会话，四项逐条）+ **对会话**段（① 声明地址 ② 显式寻址 ③ 越界须先备份，三项逐条）+ `判断方式：` 行（两段分写、违反由事后核查判定） | F19 验收 1~5（两类主体分写、与相邻卡逐条不冲突） | F19 |
| **N-08** | `### 隔离边界声明`「不覆盖」清单（`:212-215`） | 补写 | 增一条：**cwd 不在隔离范围内**——隔离覆盖的是工作树 / 索引 / HEAD 三层，cwd 从来不是隔离的一部分，也不是归属判据；不覆盖它的后果是"会话在哪儿启动与其产物落在哪儿无关" | F12 验收 1（显式声明缺失即不通过） | F12 / F17 |
...
| **N-13** | `### 状态追踪协议` 的 `status.md` 格式块（`:549-560`；字段行 `:554-559`）**+ `### 启动工作流` 步骤 2（`:330`）** | 新增字段 + 补写写入时点 | ① 格式块：在 `**迭代分支**: iteration/{迭代ID}` 行（`:557`）之后新增 `**工作区地址**: <绝对路径>`（投影 1，迭代级）；② **写入时点 = 「启动工作流」步骤 2（创建 status.md、初始化阶段表时一并写入）**——因为该字段在**开工时**（按「规则 D」解析 / 声明）即已知，且**阶段 1 的派发简报**就要带该地址（F08 验收 1），若等到 4.5 才落盘则阶段 1 期间投影 1 缺失；**与 4.5 的分工**：4.5 **只写 `**迭代分支**` 字段**（逐字对应 N-18），**不写 `**工作区地址**`**。（**D-4 的闭合落点**，2026-09-12） | W11 / F07 验收 1、F08 验收 1、F20 验收 3；`0020/status.md` 已有同形字段（既有实证，1.2-I）；**不新增步骤号**（步骤体系仍 1~5 + 4.5，F13 验收 4） | F07 / F08 |
...
| **N-18** | 同节第 4.5 步（`:333`） | 改写（**只写 `**迭代分支**` 字段**） | 删「在**会话启动之前**完成」的时点与执行者预设；改为「迭代工作区与迭代分支由会话按其工作区地址建立（缺省时，见「规则 D」）；本步骤**只负责写入 `status.md` 的 `**迭代分支**` 字段**（**`**工作区地址**` 字段的写入归步骤 2，见 N-13**），此后阶段 2~5 期间**该迭代的工作区**保持检出该分支」；步骤号与触发时机保留。**与 §8.1-M 逐字一致**（D-4 的闭合落点，2026-09-12） | F01 验收 3（会话自建）+ F13 验收 2 ②（阶段 1~6 检出迭代分支）字面同时成立 | F01 / F13 |
...
> **新增三节的位置口径（单一真源；D-10 的闭合落点，2026-09-12）**：三节**连续插入**在 `### 规则 G` 末行（`:206`）之后、既有 `### 隔离边界声明`（`:208`）之前，**次序固定为 `### 规则 H` → `### 协议产物` → `### 约束主体`**；其中 **`### 协议产物` 紧邻 `### 规则 H` 之后**（= 位置口径的生效表述，N-06）。「三节整体位于 `### 约束生效范围`（`:244`）之前」**是该口径的推论**（三节都在 `:208` 之前，而 `:208`~`:244` 之间为既有节）——它**不再是独立口径**，本文件不再以两种措辞表述同一约束（消除"放宽"读法）。**阶段 4 拆 PR 与阶段 5 施工一律按本行执行**；`prs/*.md` 的相应表述应同步为本口径。
...
| **K-09** | `:4` / `:30` / `:60` + `data/` | 改写 + 新建 | 三处版本引用（实测 `grep -n 'v0\.9\.0' SKILL.md` → `:4` / `:30` / `:60`）→ `workflow-pb v0.10.0` / `**版本**: 1.13.0（对应规范 workflow-pb v0.10.0）`；新建 `data/skill-optimization-v1.13.0.md`；`memory.md` 追加索引行。**判据**：改后 `grep -n 'v0\.9\.0' SKILL.md` = **0 命中**（同 0019 ADR-2 的 `v0.8.0` 判据）。**本节为实测口径**（Gate r2 D-3 已闭合；校正记录与旧值见 §12 的「Gate r2 项 1 / 项 2 / 项 3 已闭合」行） | —（版本一致性） | F15 / F18 |
...
| # | 文件 | 处置 | 内容 | 卡片 |
|---|---|---|---|---|
| **O-01** | `roles/workflow-pb/data/workflow-pb-changelog.md` | 追加 | `## v0.10.0（2026-09-12）` 条目（**7 段**）—— **段构成、段序依据、与规范侧 6 段的关系、两处唯一共同点，均见 §8.3 表 A / 表 B**（本行不复述） | F11 / F15 / F16 |
| **O-02** | `roles/workflow-pb/memory.md` | 追加索引行 | **在既有最新版本行之上（置顶）插入** v0.10.0 索引 —— 落点 = **`:8`（v0.9.0 行）之前**（实测：frontmatter `:1-4`、标题 `:6`、空行 `:7` ⇒ **不是文件首行**）；**Gate r2 项 3（D-4）的闭合落点** | F15 |
...
| `workflow-pb.md` 的 v0.2.0~v0.9.0 变更说明（`:45-113`）、`### 约束生效范围`（`:244-246`）、并发槛位算法（`:394-401`）、状态符号与槛位状态取值定义（`:592` / `:594` / `:596`） | 历史记录不改（改写即伪造历史）；F13 验收 4/6/7 |
...
| **L2-6** | 规范侧新增三节（`### 规则 H` / `### 协议产物` / `### 约束主体`）+ 补写 `### 隔离边界声明`，**全部沿用既有「条款 + 紧跟一行 `判断方式：`」体例**，插入位置按 §3.1 的「**新增三节的位置口径（单一真源）**」（三节连续、次序 H → 协议产物 → 约束主体、位于 `### 隔离边界声明` 之前 ⇒ 必在 `### 约束生效范围` 之前） | 0019 L2-4 先例（新增 5 条同体例规则 = L2）；体例有 8 处既有先例（§1.2-F）；位于生效范围句之前 ⇒ F13 验收 6 零改动成立；**单一口径**消除 D-10 的两种措辞 | 新增 `## 寻址契约` 独立大章节（结构层变化，0019 L1-2 已裁定"不需要新增结构层"）；塞进 `## 调度指南`（契约与调度动作混层） |
| **L2-7** | 规则 D **更名**为 `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）`，并**保留槽位与编号 D** | 内容主体已从"启动门"变为"地址解析与建立 + 显式寻址"；保留编号 D 使 0019 的引用链（规则 D 判据、硬停形态）与 SKILL 的 `§规则 D` 引用仍可定位；括注使 F11 验收 3（不得读成整条废止）可裸判定。**用户裁决（L1-1 ②，2026-09-12）采纳本决策并给出标题用语「启动与寻址契约」** | 保留旧标题「启动契约」（名实不符，读第 1 句就是"启动位置不受约束"）；预号重排（破坏既有引用链） |
...
### 6.2 L1 决策清单（**三条已确认并落盘、一条由主 agent 裁定维持 L2**；2026-09-12）

> **确认日期**：2026-09-12。「用户裁决原文」列为主 agent 逐字转达的原文；「落定形态（生效口径）」列即最终生效口径。**三条 L1 均已落盘** ⇒ 阶段 4 拆 PR、阶段 5 施工可直接按 §3 变更面清单与 §7 / §8 的措辞执行。**L1 的确认不改变"本阶段不做实现"**：规范 / SKILL / 产物仍零改动（§13）。
> 三条可合并为一次确认（同一决策的三个面：① 实态怎么改；② 载体落在哪；③ 怎么声明取代）——本次即按合并确认。

| # | 候选 | 分级判定 | 依据 | 落定形态（生效口径） | 用户裁决原文（2026-09-12） |
|---|---|---|---|---|---|
| **L1-1** | ① 把「启动契约」从 cwd 判据改写为「声明 + 寻址」= **改变框架的会话入口契约**（并删除既有启动门） | **L1 → 已确认 = 按推荐（三项全采纳）** | 与 0019 L1-1（改写角色来源 CRITICAL = 核心契约变更）同族；删除的是既有**强制机制**（发现即停的启动门）；影响所有宿主与所有会话的开工方式 | ① 删除启动门（`:169-171` 三条 cwd 判据不再作为准入条件）；规则 D 改写为「声明 + 寻址」（§8.1-D）；② 规则 D **保留编号 D 并改名**为 `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）`（§3.1-N-04 / §8.1-D，连带同步见下表）；③ 硬停输出块**删掉「创建命令」行、改为三行事实陈述**（§5.4：无指向人的动作要求、无命令块）；④ 新增 `### 规则 H：显式寻址纪律`（§8.1-H） | 「**L1-1 = 按推荐（三项均采纳）**：① 同意**删除启动门**（三项 cwd 判据不再作为准入条件）并把规则 D 改写为「声明 + 寻址」；② 规则 D **保留编号并改名**（如「启动与寻址契约」）；③ 硬停输出块**删掉「创建命令」行、改为三行事实陈述**（无指向人的动作要求、无命令块）；新增规则 H（显式寻址纪律）。」 |
| **L1-2** | ② 新增被跟踪的项目级协议产物 `docs/worktrees/README.md` 作为「工作区间 + 生效规范」的声明载体 = **新的文档类别与落点约定**，契约载体由"规范单一真源"扩为"规范（契约真源）+ 产物（实例化投影）"二层 | **L1 → 已确认 = 按推荐** | 与 0019 L1-3（把 `.pb-agents/` 与安装脚本从"运行前提"降级为"可选分发手段" = 改变既有核心流程职责）同量级：本次是**新增一个被跟踪的项目级声明面**并改变"会话从哪读工作区间"的入口 | 产物落点与文件名按 P-17④ / W5 既定（`<项目根>/docs/worktrees/README.md`，被跟踪、不改 `.gitignore`）；**5 节骨架、不含命令块与动态地址**（§4.2 即为生效形态，P-01）；规范侧**新增 `### 协议产物` 一节**承载分工与模板（§8.1-A，N-06）；契约载体 = 「规范真源 + 实例化投影」二层（§4.3） | 「**L1-2 = 按推荐**：新增被跟踪的项目级协议产物 **`docs/worktrees/README.md`**（5 节骨架，不含命令块与动态地址）；规范新增 `### 协议产物` 一节；契约载体成为「规范真源 + 实例化投影」二层。」 |
| **L1-3** | ③ 规则 C / 规则 D 的**部分条款被取代**（supersede）= 对刚交付版本（v0.9.0）的**回退式修订** | **L1 → 已确认 = 按推荐（含另两项补充）** | 被取代的是 v0.9.0 用户已确认过的条款（0019 L1-1 / L1-5 的产物）；取代必须逐句显式声明（F11），且读者可能仍持旧句当判据 | §7.1 被取代 **5 项逐句照准**（判据 1/2/3 作为启动门、`:151` 首句、`:161` 时点与后半句、硬停形态块、启动工作流首段）；§7.2 保留 **6 组照准**（P1~P6）；**另两项补充一并采纳**——⑤′（`:167` / `:200` 的子 agent 口径改写，落 N-11）与 ⑤″（`:333` 时点句 / `:447` 主体限定，落 N-18 / N-20）；术语口径 1 处（`:425` / `SKILL:125` 改称「流程结构错误中止」，落 N-19 / K-04） | 「**L1-3 = 按推荐（含另两项补充）**：§7.1 被取代 5 项逐句照准；§7.2 保留 6 组照准；另两项补充（`:425` 类改称「流程结构错误中止」、术语口径 1 处）一并采纳。」 |
| **L1-4** | ④ 主工作区定位重述（可作启动点 / 不得作工作区 / 写入仍受禁令） | **L2（主 agent 裁定维持）** | 三条之中两条是**既有语义的逐字保留**（`:153` `:196-198`），新增的唯一内容是"显式允许在主工作区启动"这一**允许性表述**；不引入实体、不改模块职责、不改系统边界。先例：0019 L1-4 | 见 §8.1-C 第 1 段（新增允许性表述）+ §7.2-P4（「不作为任何迭代的会话工作区」逐字保留）+ 规则 F 正文逐字不动 | 「**L1-4 维持 L2**（主工作区定位重述——两条既有语义逐字保留、新增仅一条允许性表述，沿用 0019 L1-4 的先例）。」 |

**三条落定的连带同步（v1.1.0；逐条给出结论）**：

| 受影响项 | 同步结论 |
|---|---|
| 规则 D 的标题与措辞 | **已同步（影响面 4 处）**：§3.1-N-04、§8.1 标签图例、§8.1-D 的标题与条款首行统一为 `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）`（**保留编号 D**）；§6.1-L2-7 与技术选型行同步；§4.2 的 §3 引用行、§1.4 组件图节点文字同步；卡 **F01 / F09** 的架构落地段同步（其余卡只引用「§规则 D」，无需改字） |
| 硬停输出形态 | **已同步**：§5.4 的三行事实陈述块即生效形态（无「创建命令」行、无指向人的动作要求、无命令块）——L1-1 ③ 的采纳结果；被取代清单第 ④ 项（§7.1）与该形态互为对照 |
| 变更面清单 | **无变化**：`N-01~N-21` / `K-01~K-10` / `P-01` / `O-01~O-04` 的条目数与承载卡不变——三条 L1 的确认只把"推荐"转"生效"，**不新增 / 不删除任何改动条目**（唯一的字面同步即上表第 1 行的规则 D 标题） |
| 回归核对的基线（T-13） | **无变化**：基线仍为 `main` @ `dbc99b5`（= v0.9.0 状态）；归类编号空间仍为 `N/K/P/O`（§3.6） |
| §12 一致性自查 | **已同步**：原「L1 已列出、未实施」行改写为「**L1 已确认且落盘、仍未实施**」，并新增「变更面无残留未裁决表述」一行 |
| T-16 术语取舍 | **无变化**：仍为「被声明对象统一称『迭代工作区』+ `会话工作区` 保留为既有标识 + 一次等同声明」；规则 D 的新标题用「启动与寻址契约」而非术语性命名，不与此冲突 |

**结论**：本迭代触发 **3 条 L1 且全部经用户确认**（L1-1 / L1-2 / L1-3，2026-09-12）；**1 条由主 agent 裁定维持 L2**（L1-4）。L1 的确认只把推荐形态转为生效口径，**不改变任何改动条目、不改变回归基线**；规范 / SKILL / 产物**一个字节都未改**（§13 自证）。
...
| **P6** | **规则 F 的实质语义与闭集例外两项**、规则 A / 规则 B / 规则 E / 规则 G、收口三步与前置游离动作、`### 约束生效范围`、阶段定义表、并发算法、PR 七字段、状态与历史记录格式 | `:192-206` / `:137-147` / `:186-190` / `:440-455` / `:244-246` / `:114-131` / 阶段 5 并发调度与槛位算法 `:379-401` / `:250-321` / `:545-685`（除 N-13 新增字段） | F13 验收 3/4/7 / N1 / N2 |
...
> **标签图例（本文各处引用 `§8.1-X` 即指下列条目）**：**C** = 规则 C（改写）；**D** = 规则 D（更名 → `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）`）；**H** = 规则 H（新增）；**A** = 协议产物（新增）；**S** = 约束主体（新增）；**G** = 隔离边界声明 补写；**I** = 文档路径协议 基准句；**J** = status.md 格式新增字段；**K** = brief 字段块新增行 + 恢复派发句；**L** = 阶段 5 步骤 2；**M** = 启动工作流 首段与 4.5；**N** = 硬停术语口径。
...
**D. `### 规则 D：启动与寻址契约（原「启动契约」改写）`**（改写；L1-1 已确认）
...
**J. `### 状态追踪协议` 的 status.md 格式块（N-13）**：在 `**迭代分支**: iteration/{迭代ID}` 行（`:557`）之后插入

```markdown
**工作区地址**: {该迭代的迭代工作区绝对地址，见「规则 D」}
```

**写入时点（N-13 的第二半；D-4 的闭合落点）**：该字段由「启动工作流」**步骤 2**（创建 `status.md`、初始化阶段表时）写入——取值 = 会话开工时按「规则 D」解析并声明的地址（简报的显式覆盖值优先）。**第 4.5 步只写 `**迭代分支**` 字段，不写本字段**（见 §8.1-M 的两块措辞）。
...
**M. `### 启动工作流` 首段、步骤 2 与 4.5（N-13 / N-17 / N-18；D-4 闭合后的单一措辞）**：
...
**步骤 2（N-13 的第二半：`**工作区地址**` 的写入时点）**：

```markdown
2. 创建 `docs/iterations/{编号-迭代名}/status.md`，初始化阶段表，并写入 `**工作区地址**: <该迭代的迭代工作区绝对地址>`
   （取值 = 会话开工时按「规则 D」解析并声明的地址；简报的显式覆盖值优先）。
```

**步骤 4.5（N-18：只写 `**迭代分支**` 字段——与 §3.1-N-18 的行文逐字一致）**：

```markdown
4.5. 阶段 1（需求收敛）通过推进条件后，在 status.md 头部写入 `**迭代分支**: iteration/{迭代ID}`。迭代工作区与迭代分支由会话
按其工作区地址建立（缺省时，见「规则 D」）——本步骤不执行 `git branch` / `git checkout`，只负责该字段的写入。
...
- **K-09**：**`:4`** description 的「（v0.9.0）」→「（v0.10.0）」；`:30` → `**版本**: 1.13.0（对应规范 workflow-pb v0.10.0）`；**`:60`** →「按 workflow-pb v0.10.0 规范调度」。**（Gate r2 项 2 / D-3 已闭合：**本节行号以实测为准**，旧值与校正说明只出现在 §12 的偏差闭合行（全文唯一一处）；实测命令 `grep -n 'v0\.9\.0' .claude/skills/workflow-pb/SKILL.md` 输出 `4:` / `30:` / `60:` —— `:4` = frontmatter description 正文、`:30` = 版本行、`:60` = Purpose 正文；**被替换的两行**（description 声明行 / `## Purpose` 标题行）见 §12 的偏差闭合行 ⇒ 以实测口径为准，与 §3.2-`K-09` 一致。）** 另：`K-09` 内含**两条落点** —— ① 三处版本引用（归 pr-001）；② 新建 `data/skill-optimization-v1.13.0.md` 与 `memory.md` 索引行（归 `O-04` / pr-003）—— 阶段 6 逐 hunk 归类时按此拆分说明，**不视为"无法归类的 hunk"**（Gate r2 D-8 的承接说明）

### 8.3 v0.10.0 变更说明组织形态（T-07 / T-12）

**段构成（两处**各自独立**；**本节表 A / 表 B 是唯一权威表述**，其余各处只写「见 §8.3」或「见 §8.3 表 X」，**不复述段序**——主 agent 裁定，2026-09-12，L2）**：

**表 A · 规范侧 `## v0.10.0 变更说明（相对 v0.9.0）`（写在 `roles/workflow-pb/workflow-pb.md` 内；落点：`:73` 之后、`:75` 之前）= 6 段**：

| 段 | 内容 | 判据 |
|---|---|---|
| ① 概述（做了什么） | 一段话概述：入口绑定 → 寻址绑定 + 契约载体化（`docs/worktrees/README.md`）+ 硬停收窄 + 人的强制面归零 + 检测挂现场；**不复述逐条改动** | 与 v0.9.0 先例同构（其首段 `:67` 为「**新增会话工作区层与隔离边界声明**：…」概述段） |
| ② 目的 | 用户对 v0.9.0 启动契约的异议（引原话要点）+ 三条一手反证（`0017/history.md:204`、0019 D-5、0020 D-1） | 与 v0.9.0 先例同构（其第 2 段 `:69` = 「**目的**：…」） |
| ③ **被取代清单（逐句）+ 保留清单** | §7.1 的 5+2 行表 + §7.2 的 P1~P6 段（**句级**，逐句引 v0.9.0 原句） | F11 验收 1~3（E8）；**本迭代插入段** |
| ④ 不改变的部分 | 阶段数量与职责 / 推进条件；并发槛位算法；PR 七字段；规则 A / B / E / G 与规则 F 的实质语义；状态与历史记录协议格式（除新增字段）；`tools/install-pb-agents.sh` 与 `.gitignore`；任何执行角色文件 | 与 v0.9.0 先例同构（其第 3 段 `:71`）；F13 验收 3/4/7 |
| ⑤ 生效范围 | 沿用既有 `### 约束生效范围` 句（新条款追加在其之前 ⇒ 自动覆盖）；不追溯已完成迭代；0018 文件零改动 | 与 v0.9.0 先例同构（其第 4 段 `:73`）；F13 验收 6 |
| ⑥ **备案** | 三段登记（与表 B 第 ⑦ 段**内容同源**）：0018 活指令失效登记 / 0019 E 项三分类中「新增两条」的登记 / D-7 留痕指引 | F15 验收 1~3（MI-07）/ F16 验收 2~3；**本迭代新增段** |

> **段序依据（规范侧；v1.1.3 裁定的落点）**：v0.9.0 的 `## v0.9.0 变更说明（相对 v0.8.0）`（`roles/workflow-pb/workflow-pb.md` `:65-73`，实测）= **概述**（`:67`）/ **目的**（`:69`）/ **不改变的部分**（`:71`）/ **生效范围**（`:73`），共 **4 段**（v0.8.0 同形：`:57` / `:59` / `:61` / `:63`）；本迭代**插入**第 3 段「被取代清单（逐句）+ 保留清单」（F11）、**新增**第 6 段「备案」（F16 验收 2）⇒ **① ② ④ ⑤ 与 v0.9.0 同构、③ 为插入段、⑥ 为新增段**。**规范侧不含「具体改动」段** —— v0.9.0 先例即如此（逐条改动只写在 changelog 文件里，规范侧从不复述）。

**表 B · changelog 条目 `## v0.10.0（2026-09-12）`（写在 `roles/workflow-pb/data/workflow-pb-changelog.md` 内）= 7 段**：

| 段 | 内容 | 判据 |
|---|---|---|
| ① 触发 | 同表 A 第 ② 段口径（迭代 + 用户异议 + 一手反证） | 与 v0.9.0 条目同构（其 `:5`） |
| ② 方案 | 五条（入口绑定 → 寻址绑定 / 契约载体化 / 硬停收窄 / 人的强制面归零 / 检测挂现场） | 同构（其 `:7`） |
| ③ **被取代清单（逐句）+ 保留清单** | 同表 A 第 ③ 段（指向 §7.1 / §7.2） | F11 验收 1~3；**本迭代插入段** |
| ④ 未改变的部分 | 同表 A 第 ④ 段口径 | 同构（其 `:17`）；F13 验收 3/4/7 |
| ⑤ 生效范围 | 同表 A 第 ⑤ 段口径 | 同构（其 `:19`）；F13 验收 6 |
| ⑥ **具体改动** | 文件—节位的逐条对应：规范侧 `N-01`~`N-21`（§3.1）、SKILL 侧 `K-01`~`K-09`（§3.2）、新建产物 `P-01`（§3.3）、记录文件 `O-01`~`O-04`（§3.4） | 同构（其 `:21`）；F13 验收 3；**第 6 段固定为「具体改动」** |
| ⑦ **备案** | 与表 A 第 ⑥ 段**内容同源**（三段登记） | F15 / F16；**本迭代新增段** |

> **段序依据（changelog 侧；Gate r2 项 1 的闭合落点，同源偏差 D-1 / D-2）**：v0.9.0 条目（`roles/workflow-pb/data/workflow-pb-changelog.md`，实测）= 触发（`:5`）/ 方案（`:7`）/ 未改变的部分（`:17`）/ 生效范围（`:19`）/ 具体改动（`:21`），共 **5 段**；本迭代**插入**第 3 段、**新增**第 7 段 ⇒ **① ② ④ ⑤ ⑥ 与 v0.9.0 同构、③ 插入段、⑦ 新增段**；**第 6 段固定为「具体改动」**，备案**不得**占用第 6 段（这正是 Gate r2 D-1 / D-2 段数口径矛盾的根因）。

> **两处的唯一共同点 = 备案段内容同源**（上述三段登记）；**段序与段数各自独立：规范侧 6 段 / changelog 侧 7 段**。v1.1.2 曾写「规范侧与 changelog 条目**共用同一段序**」，该表述**已作废**（主 agent 裁定，2026-09-12，L2）。
**其余落点（延续既有惯例，不新建文档类型）**：

| 文件 | 动作 |
|---|---|
| `roles/workflow-pb/data/workflow-pb-changelog.md` | 追加 `## v0.10.0（2026-09-12）` 条目 —— **段构成与段序依据见本节表 B 与其后的「段序依据（changelog 侧）」行**（本表不复述）；**与规范侧 6 段不共用段序**，两处唯一共同点 = 备案段内容同源 |
| `roles/workflow-pb/memory.md` | **在既有最新版本行之上（置顶）插入** v0.10.0 索引 —— 落点 = **`:8`（v0.9.0 行）之前**（frontmatter `:1-4` / 标题 `:6` / 空行 `:7`，**不是文件首行**）；**Gate r2 项 3（D-4）的闭合落点** |
| `.claude/skills/workflow-pb/data/skill-optimization-v1.13.0.md`（新建） | 变更点 / 未改变的部分 / 一致性核对 / **更正登记**（D-7 两处就地更正留痕，F18 验收 3） / 关联文件 |
| `.claude/skills/workflow-pb/memory.md` | **在既有最新版本行之上（置顶）插入** v1.13.0 索引 —— 落点 = **`:5`（v1.12.0 行）之前**（标题 `:1` / 空行 `:2` / 引注 `:3` / 空行 `:4`，**不是文件首行**）；**Gate r2 项 3（D-4）的闭合落点** |
...
| **规范侧新增 3 节 + 1 处补写（规则 H / 协议产物 / 约束主体 / 隔离边界声明）** | F02、F03~F05、F19、F12、F17 各自的"条款存在"验收 | 沿用既有「条款 + 紧跟一行 `判断方式：`」体例（8 处先例）；插入位置按 §3.1 的「**新增三节的位置口径（单一真源）**」⇒ 三节必在 `### 约束生效范围` 之前 ⇒ 不追溯效果零改动成立（**此处不再复述位置口径的第二种措辞**，D-10 的单一措辞要求） |
...
| **T-08** | 新增 / 改写条款在规范与 SKILL 中的落点与章节命名 | §3.1（N-01~N-21）+ §3.2（K-01~K-10）+ §8.1 / §8.2 全文；**新节命名** = `### 规则 H：显式寻址纪律`、`### 协议产物：工作区间与生效规范（docs/worktrees/README.md）`、`### 约束主体`；规则 D 更名 `### 规则 D：启动与寻址契约（原「启动契约」改写，v0.10.0）` 并**保留编号 D**（L1-1 ② 已确认，2026-09-12） | F01 / F02 / F08 / F09 / F10 / F14 / F19 |
...
| 检查项 | 结论 |
|---|---|
| **L1 决策已确认且落盘、仍未实施** | ✅ §6.2：**L1-1 / L1-2 / L1-3 经用户确认（2026-09-12，"按推荐"逐条原文落表）**，L1-4 由主 agent **裁定维持 L2**；三条落定的连带同步逐条给出结论（§6.2 末表）。**L1 的确认只把推荐转生效，未改变任何改动条目（N/K/P/O 条目数不变）与回归基线** ⇒ 阶段 4 可直接按 §3 / §7 / §8 执行。**仍未实施**：规范 / SKILL / 产物 / 脚本 / `.gitignore` 零改动（本阶段写入 = `architecture.md`（**v1.1.1**）+ 21 张卡的架构落地段） |
| **变更面无残留未裁决表述** | ✅ 全文检索「待确认 / 待裁决 / 待主 agent 转呈 / 需要用户回答」：§6.2 已整体改写为裁决落盘表（原"需要用户回答什么"列由"用户裁决原文"列替代）；§5.4 硬停形态、§7 取代与保留清单、§8.1-D 标题、§11 均已是生效形态；**无残留未裁决表述**（阶段 4 拆 PR 时不需要再回阶段 3 取裁决） |
| **Gate 偏差 D-4 已闭合（第 4.5 步字段写入职责）** | ✅ **根因 = 职责描述错**（**不是**行号引用错、**不是**两处说的两件事）：v1.0.0 的 N-13 只写了 `**工作区地址**` 在格式块中的**位置**、未写**写入时点**，我在 §8.1-M 顺手把该字段的写入挂到第 4.5 步；而 4.5 的执行时点是「阶段 1 通过推进条件后」，与"地址在**开工时**即声明"（阶段 1 的派发简报就要带它，F08 验收 1）冲突。**修正（单一措辞）**：`**工作区地址**` 的写入归「启动工作流」**步骤 2**（创建 `status.md`、初始化阶段表时），**4.5 只写 `**迭代分支**` 字段**。**落点 = §3.1-N-13**（语义列第 ② 项，含分工与理由）+ **§3.1-N-18**（语义列加相互指代）+ **§8.1-J**（新增"写入时点"段）+ **§8.1-M**（重写为两块：步骤 2 块 + 4.5 块；4.5 块与 §3.1-N-18 的行文**逐字一致**）。**所据行号** = v0.9.0 `:330`（步骤 2 原文「创建 `docs/iterations/{编号-迭代名}/status.md`，初始化阶段表」）与 `:333`（4.5 原文「…只负责该字段的写入」——"该字段"即 `**迭代分支**`） |
| **Gate 偏差 D-10 已闭合（新增三节的位置口径）** | ✅ **裁定 = 采「紧邻 `### 规则 H` 之后」为 `### 协议产物` 节的位置口径**（主 agent L2）：① 可逐条读序判定，比"边界"口径强；② 「三节整体位于 `### 约束生效范围`（`:244`）之前」是该口径的**推论**，**不再作为独立口径重复表述**（消除"放宽"读法）。**落点 = §3.1 表后新增的「新增三节的位置口径（单一真源）」段**（次序 `规则 H → 协议产物 → 约束主体`、插入窗口 `:206`→`:208`）+ **§3.1-N-05 / N-06 / N-07** 的位置列 + **§6.1-L2-6** 的插入位置改写。**兼容性声明**：卡 F19 的表述（"紧邻 `### 协议产物` 之后、`### 约束生效范围` 之前"）与本口径**相容**（同句既含次序又含边界），本轮按"只改 architecture.md"的指令**未改该卡** |
...
| **Gate r2 项 1 / 项 2 / 项 3 已闭合（段构成裁定 / D-3 / D-4）** | ✅ ① **changelog 条目段构成**（同源偏差 Gate r2 D-1 / D-2 / D-5）：定为 **7 段**（段序见 §8.3 **表 B**）—— 落点 = §8.3 表 B + 「段序依据（changelog 侧）」行（逐段标注 v0.9.0 实测段序 `:5`/`:7`/`:17`/`:19`/`:21`，并写明"③ 为插入段、⑦ 为新增段、第 6 段固定为具体改动"）+ §3.4-`O-01` + §8.3 落点表的 changelog 行；**v1.1.3 起规范侧段序（6 段，表 A）与本条分列**，本条只保留 changelog 侧口径；同时校正原「备案 = 第五段」的序号标注（D-5 的 by-product）。② **D-3（行号不一致）**：以**实测口径 `:4` / `:30` / `:60`** 为准（`grep -n 'v0\.9\.0' SKILL.md` → `4:` / `30:` / `60:`；`:3` 为 `description: |` 行、`:58` 为 `## Purpose` 标题行）——落点 = §8.2-`K-09`（含实测命令与输出要点）+ §3.2-`K-09`（标注为实测口径）。③ **D-4（memory.md 插入点）**：以实测「**既有最新版本行之上（置顶）**」为准 —— `roles/workflow-pb/memory.md` 的 `:8` 之前、`.claude/skills/workflow-pb/memory.md` 的 `:5` 之前；落点 = §3.4-`O-02` + §8.3 落点表的两行 |
| **Gate r2 其余偏差的本文件侧承接** | ✅ **D-5** 随段构成裁定校正（同上）；**D-6**（"头部 4 行"的计数与枚举）**architecture 侧无需改** —— §4.1 与 F04 卡的口径已是「H1 标题 + 4 个字段行」（`**状态**` / `**版本**` / `**日期**` / `**演进自**`），pr-002 的括注由阶段 5 按本口径落盘；**D-8**（`K-09` 打包为一条）**已加拆分说明**（§8.2-`K-09` 末段：三处版本引用归 pr-001、新建记录与索引行归 `O-04` / pr-003 ⇒ 阶段 6 归类不视为"无法归类的 hunk"）；**D-7**（pr-001 验收条目的交叉引用署名）属 `prs/*` 侧，按本轮指令**未改** |
| **段序裁定已闭合（v1.1.3：规范侧与 changelog 条目分列）** | ✅ 主 agent 裁定（2026-09-12，L2）：**两处段序独立** —— 规范侧 = **6 段**、changelog 条目 = **7 段**；**唯一共同点 = 备案段内容同源**（段名、逐段依据与 v0.9.0 先例行号见 §8.3 表 A / 表 B 与两行「段序依据」）。**落点** = §8.3 表 A / 表 B（唯一权威表述）+ §3.4-`O-01`（只写段数并指向表 B）+ §8.3 落点表 changelog 行；v1.1.2 的「共用同一段序」表述**已作废并就地标注** |
| **结构类表述已收敛为单一权威（L2 决定）** | ✅ 按主 agent 指示执行：本文件对**任何结构**（段序 / 编号区间 / 位置口径）只保留**一处**权威表述，其余处只写「见 §X」**不复述** —— 已核对四处：① 段构成 → §8.3 表 A / 表 B（§3.4-`O-01` 与落点表仅指向）；② 新增三节的插入次序 → §3.1「新增三节的位置口径（单一真源）」段（`N-05` / `N-06` / `N-07`、§6.1-L2-6、§10 均只引用）；③ 变更面编号空间 → §3 开头的编号制说明（`N/K/P/O`；§3.6 只引用）；④ 唯一硬停面与输出形态 → §5.4（`N-04` / `K-01` / `K-02` / `K-08` 只引用） |
| **Gate r3 偏差已闭合（残留双写法全文件清零）** | ✅ **扫描覆盖范围与结论（v1.1.4，主 agent 裁定 L2）**：对全文 **13 节**（近 1000 行）按四类事实逐一扫描 —— **① 行号类**：`workflow-pb.md` 与 `SKILL.md` 的全部行号引用（`§2` 出现点清单 7 张表、`§3.1` 的 `N-01`~`N-21`、`§3.2` 的 `K-01`~`K-09`、`§3.5` 零改动清单、`§5.2`~`§5.4`、`§7` 取代/保留清单、`§8.1`~`§8.3`、`§11`）**共 6 处发现问题、全部就地统一**：① `§2.7` 行 9 的版本引用行号 → **`:4` / `:30` / `:60`**（Gate r3 的那处；同行的"仅应命中历史记录文件名"判据同时改为 **0 命中**；**旧值的唯一出现处见下**）；② `§2.3` 行 4 的 `:33/37` → **`:33/35`**（实测 `:31` 只定义规范 / `:33` 修改本文件不得侵入 / `:35` 唯一来源）；③ `§3.1-N-08` 的「不覆盖」清单 `:210-216` → **`:212-215`**（实测 `:210` 为覆盖段）；④ `§3.1-N-13` 的字段行 `:555-558` → **`:554-559`**（实测字段 = `**工作流**` `:554` ~ `**history**` `:559`）；⑤ `§3.5` 的并发算法 `:393-401` → **`:394-401`**（表头在 `:394`）、状态符号与槛位取值 `:583-596` → **`:592` / `:594` / `:596`**；⑥ `§7.2-P6` 的 `:379-401` 加限定为「**阶段 5 并发调度与槛位算法** `:379-401`」，与 ⑤ 的"算法块"范围区分（同一对象不再有两种写法）。**② 段序类**：`7 段` / `6 段` 的出现处已收敛为 §8.3 表 A / 表 B（唯一权威）+ 仅指代 —— 本轮删除 `§3.4-O-01` 与 `§12` 段序行中的**段名枚举复述**；旧表述「共用同一段序」仅作为**已作废的历史叙述**存在于 §8.3 的作废标注行（一处）。**③ 编号区间类**：`N-01~N-21` / `K-01~K-10` / `P-01` / `O-01~O-04` 全部一致；`§1.1` 两行的「改动 20 条 + 保留 1 条」/「改动 9 条 + 保留 1 条」改为**只给条数 + 指向 §3.1 / §3.2**（不再并列范围写法）。**④ 位置口径类**：新增三节的插入次序已单一口径（§3.1「新增三节的位置口径（单一真源）」），`N-05` / `N-06` / `N-07` / `§6.1-L2-6` / `§10` 只引用不复述。**结论：除"已作废的历史叙述"那一处外，全文无同一事实的第二处写法**。被替换的旧值（版本引用行号）的**唯一允许出现处 = §12「Gate r2 项 1 / 项 2 / 项 3 已闭合」行**（作"已更正的历史叙述"；§2.7 行 9 与 §8.2-`K-09` 均已改为不复现） |
| **阶段 3 完成定义** | T-01~T-16 全填 ✅（§11）；所有功能卡有技术路径 ✅（21 卡逐卡"架构落地"段）；无架构内部冲突 ✅（**27 项逐条自查**，含 12 项冲突核对、L1 落定后的连带同步、Gate r1 / r2 / r3 偏差、段序裁定与结构类表述收敛）；**L1 三条经用户确认并落盘、L1-4 维持 L2** ✅（§6.2）；变更面无残留未裁决表述 ✅ |

---

## 13. 未越界声明

- 本阶段**唯一写入**：`docs/iterations/0020-session-workspace-addressing/architecture.md`（**新建**，v1.0.0 → **v1.1.4**）+ `docs/iterations/0020-session-workspace-addressing/prd/F01~F21*.md` 的「架构落地」段（**21 处就地补全**，产品维度未改一字；v1.1.0 另做了 F01 / F09 的规则 D 标题同步）。
...
- **L1 已确认但未实施（v1.1.0）**：§6.2 三条 L1 均已落盘为生效口径（2026-09-12 用户裁决），但本阶段**仍不做实现**——规范 / SKILL / `docs/worktrees/README.md` / 脚本 / `.gitignore` **零改动**，产物本体是阶段 5 的施工结果。
- **本轮（v1.1.0）写入范围**：本文件（就地修订：版本头、§6.1-L2-7、§6.2、§3.1-N-04、§4.2 引用行、§8.1 图例与 D 节标题、§11-T-08、§12、§13）+ `prd/F01`、`prd/F09` 的**架构落地段**（规则 D 标题同步）；**未新增任何文件**，功能卡产品维度仍一字未动。
- **本轮（v1.1.1）写入范围**：**只本文件**（版本头、§3.1-N-05 / N-06 / N-07 / N-13 / N-18、§3.1「新增三节的位置口径」段、§6.1-L2-6、§8.1-J、§8.1-M、§12、§13）——按主 agent 指令**未改任何卡**（D-10 的口径与卡 F19 的表述相容，D-4 的修正落在规范侧条款与 §3.1 描述，不涉及卡内架构段）；未新增文件；未触碰 `prs/*`。
- **本轮（v1.1.2）写入范围**：**只本文件**（版本头、§3.4-`O-01` / `O-02`、§3.2-`K-09`、§8.2-`K-09`、§8.3 的两张表与新增「段序依据」行、§12、§13）——按主 agent 指令**未改任何卡与 `prs/*`**；未新增文件；未触碰规范 / SKILL / 脚本 / 角色 / 代码 / `.gitignore`。
- **本轮（v1.1.3）写入范围**：**只本文件**（版本头、§3.4-`O-01`、§8.3 的段构成分列（表 A / 表 B + 两行「段序依据」+ 共同点声明）与落点表 changelog 行、§12 两行、§13）——按主 agent 指令**未改任何卡与 `prs/*`**；未新增文件；未触碰规范 / SKILL / 脚本 / 角色 / 代码 / `.gitignore`。
- **本轮（v1.1.4）写入范围**：**只本文件**（版本头、§2.3 行 4、§2.7 行 9、§1.1 两行、§3.1-`N-08`/`N-13`、§3.4-`O-01`、§3.5 两处范围、§7.2-`P6` 限定、§8.3 无改动、§12 段序行压缩 + 新增扫描行、§13）——按主 agent 指令**未改任何卡与 `prs/*`**；未新增文件；未触碰规范 / SKILL / 脚本 / 角色 / 代码 / `.gitignore`。
