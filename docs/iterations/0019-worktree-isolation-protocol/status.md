# 工作流进度

**工作流**: workflow-pb v0.8.0
**迭代**: 0019-worktree-isolation-protocol
**当前阶段**: 独立验证（阶段 6）
**迭代分支**: iteration/0019-worktree-isolation-protocol（**已合并进 main `9b6507b`，分支已删除**；base = main @ `9ede9ea`）
**状态**: **已完成**（阶段 6 PASS；迭代分支已合并 main，分支已删除）
**history**: 开启
**前置**: 本迭代由用户临时插入并定为**最高优先级**，0018-chat-agent-subagent-protocol 已按其裁决暂停在阶段 2 已收敛处。

## 阶段状态

| # | 阶段 | 完成 | 已验证 | 备注 |
|---|---|---|---|---|
| 1 | 需求收敛 | ✅ | ✅ | demand.md **v1.3.0**；P-1~P-16 + D-2 共 17 项全部经用户裁决；`model_inferred` 归零；C-1~C-8 全部已决；输入含 0017 复盘 Phase 1（`clarifications/retro-0017-phase1.md`） |
| 2 | 功能规格 | ✅ | ✅ | prd.md v0.2.0 已收敛；15 张卡（12 需求功能点 F01~F13 + 1 声明项 F12 + 2 保证项 F14/F15）；MI-01~MI-09 经用户裁决全部按推荐；`[model_inferred]` 零残留；T-01~T-16 架构待填 |
| 3 | 技术架构 | ✅ | ✅ | architecture.md **v1.2.2**；L1 三条经用户确认（L1-1 改写 CRITICAL / L1-3 部署定位降级 / Q-1=A 阶段 1 前建工作区+分支）；L1-2/L1-4 主 agent 裁定维持 L2；T-01~T-16 全填；零新增技术组件 |
| 4 | PR 规划 | ✅ | ✅ | 2 个 PR（pr-001 协议本体 / pr-002 跨迭代 D-2 登记）；文件真交集为空、依赖图无环；Gate 三验 r1/r2 FAIL → r3 **PASS（6/6）**；输入 = architecture.md v1.2.2 |
| 5 | PR 实现 | ✅ | ✅ | 首波并发 pr-001 ∥ pr-002（两 worktree 磁盘共存）；PR 级验收：pr-002 PASS、pr-001 首轮 FAIL-1partial → 判据口径修正 → **复验 PASS（115/115）**；两 PR 均已合并（`1ca983d` / `dbef2b5`），worktree 与特性分支均清理  |
| 6 | 独立验证 | ✅ | — | **PASS（8/8 条目 pass；0 fail/partial/blocked）**——报告 `clarifications/verify-stage6-20260912-200521.md`；含「并发调度真实执行证据」三项核查全 pass、MI-05 构造样本（85 条命令）闭合 E1~E4 + R-2；偏差记录 7 条（不阻塞）、下一迭代候选 8 条  |

## PR 实现子状态（阶段 5 展开）

| PR 文件 | depends_on | 状态 | worktree 分支 | 已合并 | 槛位状态 |
|---|---|---|---|---|---|
| pr-001-workflow-pb-workspace-isolation-protocol.md | （无） | ✅ | (已清理) | ✅ dbef2b5 | 已释放  |
| pr-002-cross-iteration-d2-registration.md | （无） | ✅ | (已清理) | ✅ 1ca983d | 已释放  |

> 两者 `depends_on` 均为空 ⇒ 首波即可并发（预期并发度 = 2，关键路径 = pr-001）。
> 注：pr-002 只改 `docs/iterations/0019-…/status.md` 的 `## 待确认项` D-2 单段（ADR-4），与本文件的主线维护存在同文件并发写入——合并前须以当时迭代分支重基线。

## 并发配置（阶段 5）

**起始并发数**: 3
**硬上限**: 5（公式 `2×起始-1`）
**当前有效上限**: 5（按公式 `min(3 + 1×3, 5)` 重算，已达硬上限）
**累计槛位释放次数**: 2
**已派发总数**: 2
> **阶段 5 完成**：两个 PR 全部合并进迭代分支（`1ca983d` / `dbef2b5`）；累计槛位释放 2、当前有效上限维持 5（`min(3+2×3,5)` 触顶，已达硬上限）。
**阶段 6 偏差记录（7 条，不阻塞交付；主 agent 已登记为下一迭代候选）**

| # | 偏差 | 建议处理 |
|---|---|---|
| D-1 | 「隔离边界声明」章节无独立 `判断方式：` 行（两条非 git 资源约束的判断方式内联于表格单元格） | 下一迭代补一行独立判断方式（不影响 F13 验收 2 的通过判定） |
| D-2 | pr-002 验收条写「合并前置 = 重基线 rebase」，实际执行为 `--no-ff` merge（实质约束「以主线版为基 + 只重放 D-2 单段」经 diff 核实成立） | 下一迭代修正该验收条措辞 |
| D-3 | 槛位释放 1 的区块更新滞后该次合并约 3 分钟（`cefa78a` 19:55:30 vs `1ca983d` 19:52:32） | 属台账时序，无需返工；登记备考 |
| D-4 | 本文件 4234b0c 版的槛位公式代入仍写 `min(3+1×3,5)`（当时累计已为 2，正确为 `min(3+2×3,5)`；值 5 正确） | **已随本次收口修正**（本行下注已改为正确代入） |
| D-5 | 本迭代发生一次跨工作区写入（pr-001 dev 相对路径误写主工作区 14 次编辑），无前置备份动作——属规则 F 的活体反例；主工作区零内容损失 | 已登记为下一迭代候选：sub-agent 简报的「工作目录纪律」应成为派发模板固定字段 |
| D-6 | SKILL `:32` 变更历史行与 `:367` Resources 未含 v1.11.0/v1.12.0（经裁定刻意不改） | 登记备考；下一迭代可一并清理 |
| D-7 | `skill-optimization-v1.12.0.md` 两处写「规范 §8.4」，实为 `architecture.md` 的 §8.4 | 下一迭代修正交叉引用措辞 |
## 收口（迭代分支合并进 main）

- 合并动作：在仓库主工作区执行 `git checkout main` → `git merge --no-ff iteration/0019-worktree-isolation-protocol`（成 **`9b6507b`**，无冲突）→ `git branch -d iteration/0019-worktree-isolation-protocol`。
- 合并后冒烟：`tests/test-check-role-structure.sh` **PASS（6/6）**；`tests/test-check-model-dispatch-protocol.sh` **4 项 FAIL（预存失败，与本迭代无关）**——该脚本断言「当前 worktree 必须为 `.pb-agents/worktrees/agents-0004-model-dispatch`」（0004 遗留的陈旧检查），`grep` 证实其不涉及 `workflow-pb` / `SKILL` 任何内容；已登记为下一迭代候选。
- 迭代 ID 目录保留 `status.md` / `history.md` / `demand.md` / `prd.md` + `prd/` / `architecture.md` / `prs/` / `clarifications/` / `progress.md` 全部产物，不删除。

## PR 验证（Gate 阶段 4 → 5 入口）

| 轮次 | 报告 | 结论 |
|---|---|---|
| r1 | clarifications/verify-gate-stage4-20260912-191730.md | FAIL（2 partial：与 architecture 一致性 / 交付前置） |
| r2 | clarifications/verify-gate-stage4-r2-20260912-192704.md | FAIL（2 partial：同上，问题面收窄） |
| **r3** | **clarifications/verify-gate-stage4-r3-20260912-193332.md** | **PASS（6/6 pass，fail 0 / partial 0；偏差记录 8 条不阻塞）** |

## 待确认项

- **无用户待确认项**（P-1~P-16 + D-2 全部裁决并落盘，`model_inferred` 归零）。
- **D-1（已解决）**：用户答复「已停，继续推进」；主 agent 的「落盘即提交 + 切分支前核对」自保措施继续执行。
- **0017/D-2（跨迭代承接项）**：`GET /api/chats` 与 `POST /api/messages` 的路由登记元数据 `params` 未包含必填参数 `project_id`。
  承接方：**单独的小迭代**（`0019-worktree-isolation-protocol` 完成之后启动，本迭代不承接）。
  引用链：`0017/status.md`（缺陷登记）→ `0018/status.md`（C-3 显式不纳入；已完成的外部证据）→ `0019/status.md`（N2 显式排除并登记承接条件）。
  - **编号口径**：跨迭代引用一律写成 `<迭代号>/D-<n>`（如 `0017/D-2`）；`D-*` 在本迭代自身台账内不与 `0017/D-2` 复用同一编号（0019 自身的台账项自 `D-3` 起编号）。
- **D-3（编号冲突，待架构阶段统一）**：0017 的 `D-2`（缺陷编号）与 0019 早期使用的 `D-2`（verify-stage6 一字符差异）同号不同义；本迭代已改用 `D-*` 仅指 0019 自身台账项，跨迭代引用时将显式写明迭代号。
- **D-4（0017 收尾产物的一字符差异，备查）**：0017 `verify-stage6-20260912-173934.md` 提交版第 77 行（`695-698`）与第 160 行（`695-699`）自相矛盾，源真值在 `oamp/src/web.js` 696-697（复盘 Q-c 结论）；不属本迭代范围，仅留痕。
- **D-5（澄清记录缺档，留痕）**：`clarifications/` 下无 `round-2.md`（第二轮记录仅存于 `demand-round-2-proposals.md`，其正文自述第二轮有 2 项待裁决）；由 progress-observer 首次观测发现（F-3），内容未丢失，仅记录体例不完整。
- **D-6（progress-observer 首次观测的 6 条不一致，逐条处置）**：F-1 工作区改动 = 本迭代进行中的 history 写入（非他人改动，已随提交消除）；F-2 本文件原用词「分两次提交」与 git 事实（3 个提交）不符 → **已修正**；F-3 → 见 D-5；F-4/F-5 为引用位置说明；F-6 跨迭代项 → 0018 status 的 stash 声明与实际不符（`refs/stash` 不存在）→ **已用临时 worktree 修正 0018 status**。
- **主 agent 偏离记录（留痕）**：① `iteration/0019-…` 分支**在阶段 1 通过前即创建**（规范为阶段 1 通过后创建）——原因是用户裁决「先做 0019、暂停 0018」要求立即离开 0018 分支工作区，属操作必要偏离；② 0019 的产物在阶段 1 期间即分三次提交进本分支（`0a89a4d`、`c3b5152`、`27f280b`），理由同 D-1 自保措施。

## 用户确认记录（Gate）

- **迭代 ID**：`0019-worktree-isolation-protocol`（主 agent 拟定并采用）。
- **优先级与落地方式**：最高优先级；先做 0019、暂停 0018（用户裁决 A）。
- **P-1 = ①**：worktree 为规范默认且唯一被规范化形态；clone 仅作跨机/无写权限兜底（依据 0017 复盘 Q-a）。
- **P-2 = ①**：启动契约 + 硬停（不在本会话专属工作区 → 停止推进并输出创建命令）。
- **P-6 = ①**：合并进 main 固定在仓库主工作区执行，会话工作区永不 checkout main。
- **P-8 = ①**：角色定义读取来源改为被 git 追踪的内容（本仓库以 `roles/**` 为源），`.pb-agents/` 与安装脚本降级为面向业务项目的可选分发手段。
- **P-15 = ①（新增范围）**：非 git 共享资源（默认端口占用、负载致偶发失败、`/tmp`）纳入为最小声明与约束。
- **P-16 = ①（新增范围）**：跨工作区写入的负面约束 + 判断方式，并写明该条款不可能被 git 强制。
- **P-3 / P-4 / P-5 / P-7 / P-9 / P-10 / P-11 / P-12 / P-13 / P-14 = 全部按推荐**：每迭代一个工作区 / PR worktree 为子层 / 「主 agent 根目录」改写为「该会话的迭代工作区根目录」/ 分支名必含迭代 ID / 共享 refs 负面约束 / 接受冲突后移到合并期 / 只做文本级条款不做 hook / 暂停现场原样保留 / 只保证同机同仓库多会话 / 沿用不追溯生效。
- **D-2 = ①**：由单独小迭代承接，本迭代不承接。

## 更新日志

- 2026-09-12: 迭代启动（Step 0 前提声明：本宿主具备真实阻塞式人机交互通道 `ask`）。
- 2026-09-12: 创建迭代分支 `iteration/0019-worktree-isolation-protocol`（base = main `9ede9ea`）+ 事故一手记录 → 派发 demand 第一轮。
- 2026-09-12: 用户裁决 P-2/P-6 + 其余 10 项按推荐；P-1 要求「先复盘迭代 0017」；P-8 要求重拟方案（不依赖工具、属 git 使用规范）→ 派发 `retrospective` 对 0017 做 Phase 1 扫描。
- 2026-09-12: 0017 复盘产出（Q-a worktree 足够且更优 + 5 类残留、Q-b 零动作路径、Q-c 11 项漂移核查）→ demand 第二轮落定 12 项 + 重拟 P-1/P-8 选项。
- 2026-09-12: 用户裁决 P-1 ① / P-8 ① / P-15 ①（新范围）/ D-2 ①（另起小迭代）→ demand 第三轮收口（v1.2.0），仅剩 P-16。
- 2026-09-12: 用户裁决 P-16 ① → demand 第四轮归零（v1.3.0）→ **阶段 1 推进条件三项全部通过 ✅**；阶段 1 产物分三次提交（`0a89a4d`、`c3b5152`、`27f280b`）→ 进入阶段 2（功能规格），派发 prd。
- 2026-09-12: 自动派发 progress-observer（阶段 1 推进完成触发）→ 首次产出 `progress.md`（199 行，观测基线 `27f280b`），独立核实出 6 条不一致（F-1~F-6）与 5 条无法核实项 → 主 agent 逐条处置（本文件用词修正、0018 status 的 stash 声明经临时 worktree 修正、round-2.md 缺档留痕）。
- 2026-09-12: 阶段 2 两轮：prd v0.1.0（15 张卡 + 9 项 MI）→ 用户裁决组 A/B 全部按推荐、组 C 载体不限 → prd v0.2.0 收敛（`[model_inferred]` 零残留）→ **阶段 2 推进条件三项通过 ✅**。
- 2026-09-12: 主 agent 裁定 prd 提请的 3 项体例（E10 → 保留 F12〔声明项〕；E5/N4 → 与 W11 合并为 F13；E12/N2 → 保留 F15〔保证项·登记义务〕），均沿用既有体例先例；已落入 prd.md 与相关卡。
- 2026-09-12: 进入阶段 3（技术架构），派发 architect。
- 2026-09-12: 阶段 3 两轮：architect 产出 architecture.md（零新增技术组件、T-01~T-16 全填、3 条 L1 + 4 条 L2 高影响判断）→ 用户裁决 L1-1/L1-3/L1-5(Q-1=A) 全部按推荐 → 主 agent 裁定 L1-2/L1-4 维持 L2 → architecture.md v1.1.0（L1 三条已确认、Q-1=A 落定三处、R-1 解除）→ **阶段 3 推进条件三项通过 ✅**。
- 2026-09-12: 进入阶段 4（PR 规划），派发 pr-planner。
- 2026-09-12: 阶段 4 三轮 + Gate 三验：pr-planner 产出 2 个 PR（文件真交集为空、依赖图无环）→ Gate r1/r2 FAIL（台账引用不一致）→ architect v1.2.1/v1.2.2 + pr-planner 三轮同步 → **Gate r3 PASS（6/6）**。
- 2026-09-12: 阶段 5 首波并发：pr-001 ∥ pr-002 两个 worktree + planner → dev（提交 `c034dab` / `d038e3c`）→ PR 级验收（**pr-002 PASS**；pr-001 FAIL-1partial，判据口径过严）。dev001 发生一次相对路径跨工作区写入事故（已自行修复、主 agent 核验主工作区零损失），登记为本迭代 W12/F09 的活体实例。
- 2026-09-12: **pr-002 合并进迭代分支**（`1ca983d`，ADR-4 处置生效：`0019/status.md` 自动合并、主线段落完整保留）；pr-002 worktree 与分支已清理；槛位释放 1、当前有效上限爬升至 5。
- 2026-09-12: progress-observer 二次观测（观测基线 `1ca983d`）发现 7 条台账不一致（抬头/阶段表落后于 git 事实、architecture 版本引用滞后、pr-002 验收条的「重基线」与实际 merge 方式差异等）→ 主 agent 逐条修正：阶段 4 置 ✅ 且已验证 ✅（Gate r3 PASS）、阶段 5 置 ⏸、抬头改为阶段 5、architecture 版本引用改 v1.2.2。
- 2026-09-12: pr-001 复验 **PASS**（115/115，0 fail/partial；3 条偏差均为标准文档计数/举例面不完备，非实现缺陷）→ **pr-001 合并进迭代分支**（`dbef2b5`）→ worktree 与分支清理 → **阶段 5 全部 PR 合并完成**，阶段 5 置 ✅。
- 2026-09-12: 阶段 6 最终独立验证 **PASS（8/8，0 fail/partial/blocked）**——含「并发调度真实执行证据」三项核查全 pass（worktree 时间窗重叠 19:35–19:52 / 并发配置五字段跨四版本真实更新、累计槛位释放 0→1→2 / 爬升公式真实触发 3→5）、MI-05 构造样本 85 条命令闭合 E1~E4 与 R-2、需求 E1~E13 与 F01~F15 逐条追溯 pass；偏差 7 条不阻塞、下一迭代候选 8 条 → **阶段 6 置 ✅**。
- 2026-09-12: **迭代分支合并进 main**（`9b6507b`，无冲突）并删除迭代分支；合并后跑仓库自带检查：角色结构 **6/6 PASS**、model-dispatch 协议检查 4 项预存 FAIL（与本迭代无关，已登记）；**迭代 0019 完成**。
