...
**版本**：1.2.0（阶段 3 产物；v0.1.0 = 初版，v1.1.0 = L1 裁决落盘收口，v1.2.0 = **台账修正：阶段 4 pr-planner 上报的内部矛盾已闭合**）
**日期**：2026-09-12
**状态**：**阶段 3 完成（v1.2.0 台账已修正，内部矛盾闭合）**——L1-1 / L1-3 / L1-5 已由用户确认（逐条裁决原文见 §6）；L1-2 / L1-4 由主 agent 裁定**维持 L2**；Q-1 已裁决为 **A**（见 §7），Q-2~Q-5 为已决 L2（供知悉）。§1~§5 与 §8 为**硬契约**（工作区拓扑、命令表、判据、变更面、条款落点与措辞），阶段 4 拆 PR 与阶段 5 实现不得偏离。
**版本历史**：v0.1.0（初版：全部 L1 待确认、Q-1 待裁决）→ **v1.1.0**（2026-09-12：§6 三条 L1 转「已确认」并附裁决原文；§7 Q-1 转「已裁决 A」，结果落到 §3.2 命令 1 的执行时点、§5.1-W11、§8.1 规则 C 的建立时点；§10 R-1 解除；§12 完成定义转 ✅）→ **v1.2.0**（2026-09-12：**触发来源 = 阶段 4 `pr-planner` 上报的内部矛盾**——`roles/workflow-pb/workflow-pb.md:114`（阶段定义表·阶段 5 行）的 `worktree` 术语限定在 §2.3-#4 被判「改写」，却在 §5.1 变更面无承载条目，且被 §5.1 零改动清单的 `## 阶段定义` 判为「零改动」（同一处同时被判「要改」与「零改动」）；**裁定者 = 主 agent，L2**，方向 = 按 §2.3 加术语限定、并把 `## 阶段定义` 的零改动范围收窄为「除该术语限定外的全部内容」。落点：新增 §5.1-**W19**、§5.1 零改动清单收窄、§2.3-#4 补承载引用、§12 新增一致性行）。
...
> **③ 的判定规则（用于 F07 验收 5 的字面判据）**：某处 `worktree` 出现，若其**同句或所在小节标题**已含"PR"/"失败 / 阻塞的 PR"/"阶段 5"之一，则该处指代唯一，**不改**（避免顺手改动，F14 验收 1 的反面）；否则加限定词。按此规则，**术语加限定**的改写面 = **7 处**（规范 6 处：`:114` / `:127` / `:129` / `:135` / `:258` / `:611`；SKILL 1 处：`:303`）；此外规范 `:277` 的 `<path>` / `<分支名>` 具体化属 T-01 / T-03 的落点填充（非术语改写）；其余 **26 处**逐条给出"零改动"的理由。
...
| **L2-7** | `worktree` 术语的改写面 = **6 处**（规范 `:114 / :127 / :129 / :135 / :258 / :611` + SKILL `:303`），其余 28 处按"语境已含 PR / 阶段 5"判为可判、不改 | F07 验收 5 的判据是"出现无法判断的条款"，不是"每个 worktree 都要带前缀"；F14 验收 1 要求每处改动可归类，逐条给出"不改"的理由同样是可核对的输出 | 全文 34 处一律加限定（含历史变更说明与角色文件 ⇒ 越界） |
...
| W12 | `## 调度指南·派发执行角色时的 brief 构建`（`:236 / 244 / 246`） | `.pb-agents/roles/...` → `{角色定义根}/...`（解析后写具体路径） | 2.4-#1~#3 | F08 |
...
| W15 | `## 文档路径协议`（`:350-`） | 补一句：全部路径**相对该会话所在的迭代工作区根目录**判读 | W5 | F04 |
...
| W19 | `## 阶段定义`·阶段 5 行（`:114`，**v1.2.0 新增**） | **只**把该行「逐 PR 在**独立 worktree 分支**、独立子 agent 中执行」中的 `worktree` 限定为「**PR worktree** 分支」；**阶段定义表的结构与职责 / 输入 / 输出 / 推进条件四列一律不动**（6 阶段、全部推进条件逐字不变） | 新增会话层 worktree 后裸 `worktree` 不再单义（F07 验收 5 是**整文件扫描**判据，保留裸用法会使该项在阶段 6 无法通过）；本行是 §2.3-#4 处置的承载条目，闭合「判为要改却无承载条目 + 同处被列零改动」的矛盾 | F07 / F13 / F14 |
...
| S10 | `:20` 版本号与 `:21` 完整规范引用 | 1.11.0 → 1.12.0；`:31`/`:365` 的章节名引用同步 | F08 / F14 |
...
| `docs/iterations/0017-project-workspace/**`、`0018-chat-agent-subagent-protocol/**` 的既有产物 | 除 F15 的三处 status 登记外，零改动 |
...
| `docs/iterations/0017/status.md`、`docs/iterations/0018/status.md`、`docs/iterations/0019/status.md` | F15 三处 D-2 登记（措辞见 §8.7） |
...
| 归类判据 | 每个 hunk 必须能归到 §5.1 的 W1~W19 或 §5.2 的 S1~S10 某个编号（该编号再追溯到 F01~F13；W18 = 零改动行、W19 = v1.2.0 新增的阶段定义表术语限定）；**出现无法归类的 hunk 即 F14 验收 1 不通过** |
...
**落点**：`0017/status.md`§下一迭代候选（第 1 条改写为上述三段式）/ `0018/status.md`（C-3 处补引用链 + 承接方）/ `0019/status.md`§待确认项（现有 D-2 项改写为同一格式）。
...
| R-5 | **`worktree` 术语判定（L2-7）依赖"语境自明"，判据偏解释性** | 已在 §2.3 逐条给出判定与理由，形成可复查清单（F13 验收 2 的"未参与讨论者可判定"由此满足） |
...
| 每个改动可追溯到卡片（F14 验收 1） | ✅ §5.1 的 W1~W19 与 §5.2 的 S1~S10 逐条带 F 编号（W18 为零改动行、W19 为 v1.2.0 新增的术语限定承载条目）；§2 的清单外项逐条给出不改的理由 |
