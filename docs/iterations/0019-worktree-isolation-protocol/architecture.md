...
**版本**：1.1.0（阶段 3 产物；v0.1.0 = 初版，v1.1.0 = 3 条 L1 经用户确认 + Q-1 裁决 A 落盘收口）
**日期**：2026-09-12
**状态**：**阶段 3 完成**——L1-1 / L1-3 / L1-5 已由用户确认（逐条裁决原文见 §6）；L1-2 / L1-4 由主 agent 裁定**维持 L2**；Q-1 已裁决为 **A**（见 §7），Q-2~Q-5 为已决 L2（供知悉）。§1~§5 与 §8 为**硬契约**（工作区拓扑、命令表、判据、变更面、条款落点与措辞），阶段 4 拆 PR 与阶段 5 实现不得偏离。
**版本历史**：v0.1.0（初版：全部 L1 待确认、Q-1 待裁决）→ **v1.1.0**（2026-09-12：§6 三条 L1 转「已确认」并附裁决原文；§7 Q-1 转「已裁决 A」，结果落到 §3.2 命令 1 的执行时点、§5.1-W11、§8.1 规则 C 的建立时点；§10 R-1 解除；§12 完成定义转 ✅）。
...
| 4 | 114 | 阶段定义表 阶段 5：「逐 PR 在**独立 worktree 分支**……」 | PR 层（语境"逐 PR"已可判） | **改写（加限定）**：→「独立 **PR** worktree 分支」 |
...
**零改动（明确列出，供阶段 6 逐条核对）**：`## 阶段定义`（6 阶段表与全部推进条件）、`## PR 文件格式规范`（七字段）、`## 并发槛位算法`（公式与硬上限）、`## 验证目标`（含「并发调度真实执行证据」三项）、`## 状态追踪协议`（表头 / 状态符号 / 槛位取值 / worktree 分支列标注 / 更新时机）、`## 历史记录协议`、`### 约束生效范围`、`v0.2.0 ~ v0.8.0` 全部变更说明、`## 需用户决策的情况`、`### 终止工作流`、`### 阶段回退`、`### 可观测性`（progress-observer 触发时机与核实项）。
...
| 归类判据 | 每个 hunk 必须能归到 §5.1 / §5.2 的某个 W/S 编号（该编号再追溯到 F01~F13）；**出现无法归类的 hunk 即 F14 验收 1 不通过** |
...
| 与 N1 / N8（阶段序列、规则 A/B 实质语义不变）一致 | ✅ 阶段定义表 / 推进条件 / 槛位算法 / PR 七字段零改动；规则 A/B 仅术语加限定，`判断方式` 行逐字不动 |
...
| 每个改动可追溯到卡片（F14 验收 1） | ✅ §5.1 的 W1~W17 与 §5.2 的 S1~S10 逐条带 F 编号；§2 的清单外项逐条给出不改的理由 |
