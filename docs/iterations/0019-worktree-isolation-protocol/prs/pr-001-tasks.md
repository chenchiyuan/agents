...
**修订 r2（2026-09-12）**：① **T4 第 1 条的判据口径按主 agent 裁定修正**（原 `[model_inferred]` #2 → 转为裁定登记）——`.pb-agents/roles` 的判据范围限定为**规范性表述**，`## v0.9.0 变更说明` 中的历史叙述不在其内；② **计数口径定版**（见文末「计数口径定版（r2）」）——`^判断方式：` = **8 行**（规则集 7 + 来源与部署节 1）、PR 文件 `- [ ]` = **32 条**、S6 的「32 处」= **32 行**（35 个出现处）；③ **SKILL 零改动清单补 S6 范围排除**（T8 第 13 条 / T9 第 4 条，写法与 W12 / Gate r2 D2 同构）；④ **`[model_inferred]` #1 已裁定**（T2 第 7 条的两节位置口径 = `## 提交管理约束` 内、`### 约束生效范围` 之前，即「该标题之后」读法；裁定者 = 主 agent，2026-09-12，L3 级）——**本文件现无未确认项**。依据：`history.md` 的「2026-09-12 19:52:00 · 调度决策」条目 + `clarifications/verify-pr-001-20260912-195010.md`。

**输入**：`prs/pr-001-workflow-pb-workspace-isolation-protocol.md`（本 PR 唯一范围依据；**32 条 `- [ ]` 验收条目**，口径见文末「计数口径定版（r2）」①）+ `architecture.md` **v1.2.2**（§5.1 定版 **W1~W20** / §5.2 **S1~S10** / §5.3 零改动清单 / §5.4 惯例文件同步 / §5.5 归类判据 / §8 条款措辞 / §6.1 ADR-1~ADR-4 / §7 Q-1~Q-5）+ `prd/F01~F14*.md`
...
**口径**：每条验收标准的形态都是**文本级、可在不运行代码的情况下判通过/不通过**（条款存在性 / 措辞逐字比对 / `grep` 判据 / diff hunk 可归类）；追溯来源逐条标注在括号内。`[model_inferred]` **无未确认项**（2 条推导项均已于 2026-09-12 由主 agent 裁定并转为裁定登记），见文末。
...
**验证方法**：读 `:123-160` 区域逐条核对；`grep -n '^### 规则' roles/workflow-pb/workflow-pb.md` ⇒ 恰 7 条、顺序 A/B/C/D/E/F/G；`grep -n '^判断方式：' roles/workflow-pb/workflow-pb.md` ⇒ **全文 8 行**，口径拆分：**规则集 A~G = 7 行**（落地后 `:141`/`:147`/`:163`/`:184`/`:190`/`:200`/`:206` = **本任务**的核对面）+ `### 角色定义来源与部署` 节 **1 行**（落地后 `:242` = T2 的核对面）；其中 `:141` / `:147` 与改前逐字相同；另 `## 验证目标` 节一处 `- **判断方式**：`（带 bullet 前缀，落地后 `:517`）不满足行首精确匹配、不计入；`grep -n '^#### 判断方式' roles/workflow-pb/workflow-pb.md` ⇒ 0 命中；`git diff 9ede9ea -- roles/workflow-pb/workflow-pb.md` 该区域 hunk 逐条可归到 W2~W8。**不跑测试、不跑构建**（本 PR 为文本变更，无测试面）。
...
1. **三处路径替换**：`:236` / `:244` / `:246` 的 `.pb-agents/roles/...` → `{角色定义根}/...`（解析后写具体路径）；改动后规范内 `.pb-agents/roles` 的**规范性表述仅剩下游副本语境**（即 T2 引入的 `### 角色定义来源与部署` 节，落地后 `:234` / `:236`），brief 构建节不再出现该串；**`## v0.9.0 变更说明（相对 v0.8.0）` 中的历史叙述不属本判据范围**（落地后 `:69`——该段叙述降级前的旧状态，是 W1 要求的"目的"段内容）——（PR 验收 W12；arch §5.1-W12 / §2.4-#1~#3 / §8.4 / R-4；F08 验收 1；**主 agent 裁定，`history.md` 2026-09-12 19:52:00**）。
...
**验证方法**：读 `:234-260` 与 `:579` 逐条比对；`grep -n '\.pb-agents/roles' roles/workflow-pb/workflow-pb.md` ⇒ 命中 **3 行**：落地后 `:234` / `:236`（`### 角色定义来源与部署` 节内，下游副本语境 = **规范性表述**）与 `:69`（`## v0.9.0 变更说明`「目的」段的历史叙述，**不在判据范围**，口径见文末「计数口径定版（r2）」③）；`grep -n 'worktree 分支：' roles/workflow-pb/workflow-pb.md .claude/skills/workflow-pb/SKILL.md` ⇒ 两处均为 `PR worktree 分支：{分支名}`；diff 该区域 hunk 可归到 W12 / W20（`:579` 不得出现在 diff 中）。**不跑测试、不跑构建**。
...
6. **S6 32 处路径引用机械替换**：SKILL 全文 `.pb-agents/roles/...` → `{角色定义根}/...`；计数口径 = **行数**（基线 `grep -c` = 32 行；同一行内出现多次按 1 计，出现次数另计为 35——见文末「计数口径定版（r2）」④）；改动后 `grep -n '\.pb-agents/roles' .claude/skills/workflow-pb/SKILL.md` **仅剩**"可选分发手段 / 下游业务项目副本"语境（预期 ≤2 行，arch R-4 判据；「处」= 行）；且不存在"必须用 `.pb-agents/roles/`"与"不得读 `roles/`"并存的表述——（PR 验收 S6；arch §5.2-S6 / R-4 / §2.4 的 35 处清单；F08 验收 3）。
...
4. **SKILL 零改动面无 hunk**：其余 6 条 CRITICAL、`## Purpose`（除版本数字）、`## Success criteria`、`## Strategy`、`## Important facts`、Step 1~4 / Gate 4→5 / Step 6、`## § 对外协议` 三组契约、`## § 用户决策点与暂停格式`、`## § status.md / history.md 更新时机`、`## Safety` 其余条目——**除 S6 列明的路径引用行外**（`Step 6`、`## § status.md 更新时机`、`## § history.md 更新时机` 内各 1 行，按更具体的 S6 执行；该 hunk 归类 = **S6**，不判为「零改动面被改」；范围排除写法与 W12 / Gate r2 D2 同构）——（PR 验收「零改动面核对」；arch §5.2 零改动清单 + §2.4-#19~32；F14 验收 1、7）。
...
> **计数口径**：PR 文件 `## 验收标准` 实测 **32** 条 `- [ ]` 条目（`grep -c '^- \[ \]' prs/pr-001-workflow-pb-workspace-isolation-protocol.md` = 32）；本表为**归并视图**（28 行覆盖全部 32 条，同类验收项已归并到同一承载单元，归并关系见各行括号内的 W/S/F 编号与 arch 节号）。
...
### `[model_inferred]` 验收标准（**无未确认项**——2 条推导项均已于 2026-09-12 由主 agent 裁定，转为裁定登记如下）

1. **T2 第 7 条的位置口径**：`### 隔离边界声明` 与 `### 角色定义来源与部署` 的位置判据写为「`## 提交管理约束` 章节内、既有 `### 约束生效范围` 句**之前**」。推导依据 = PR 文件「生效范围不变」的"**新条款全部**追加在既有 `### 约束生效范围` 之前" + arch §8.6 / §4-L2-4 的"新规则追加在该句之前 ⇒ 自动被覆盖" + §8.2 落点行的"规范内，`## 提交管理约束` 之后"（"之后"取**该标题之后**的读法，与"在 `### 约束生效范围` 之前"相容）。**替代读法**：若 §8.2 的"之后"取"整节之后"义，则两节落在 `### 约束生效范围` 之后 ⇒ 与 PR 文件的"新条款全部在生效范围之前"**字面冲突**，且不被既有生效范围句覆盖（F14 验收 6 的意图落空）——该替代读法**未被采纳**。**裁定（主 agent，2026-09-12，L3 级）：口径 = 「`## 提交管理约束` 内、`### 约束生效范围` 之前」（即「该标题之后」读法）**；依据 = 与 PR 文件「生效范围不变」一致，且实现与 PR 级验收已按该字面通过（落地后 `:208` / `:230` < `:244`，见 `clarifications/verify-pr-001-20260912-195010.md` 的通过项）。**本项不再是 `[model_inferred]`**，T2 第 7 条与 T9 第 3 条按此口径定版。
2. **T4 第 1 条的口径——已裁定（不再是 `[model_inferred]`，登记为裁定）**：主 agent 于 2026-09-12 裁定 = 「改动后规范内 `.pb-agents/roles` 的**规范性表述仅剩下游副本语境；版本变更说明（`## v0.9.0 变更说明`）中的历史叙述不属该判据范围**」（依据：W1 要求变更说明写清"目的"，叙述旧状态属必要内容；裁定记录 = `history.md` 的「2026-09-12 19:52:00 · 调度决策」条目；落地证据 = `:234` / `:236` 为下游语境、`:69` 为变更说明历史叙述）。**该口径已写入 T4 第 1 条，不再列为待确认项。**

### 计数口径定版（r2）

> 本文件全部数字的最终口径与实测值（实测面 = 落地后的 PR worktree 分支 `feat/0019-pr-001-workflow-pb-protocol`，HEAD `c034dab`；基线 = `9ede9ea` / `15d5eaf`）。**「处」在本迭代一律按行计**（同一行内出现多次按 1 计）。

| # | 计数对象 | 最终数字 | 计数口径（命令形态） | 适用条目 |
|---|---|---|---|---|
| ① | PR 文件验收条目 | **32** | 行数（`grep -c '^- \[ \]'`） | 本文件头部输入行；「PR 验收项 → 承载任务」表的覆盖对象 |
| ② | 规范 `^判断方式：` 行 | **8** | 行首精确匹配（`grep -c '^判断方式：'`）；拆分 = **规则集 A~G = 7**（`:141`/`:147`/`:163`/`:184`/`:190`/`:200`/`:206`）+ **`### 角色定义来源与部署` = 1**（`:242`） | T1 验证方法（核对面 = 规则集 7）、T9 验证方法（核对面 = 全文 8）；T2 第 6 条含来源节的该行 |
| ③ | 规范 `.pb-agents/roles` 命中 | **3 行** | 行数（`grep -n '\.pb-agents/roles'`）；按**判据范围**二分 = **规范性表述 2 行**（`:234`/`:236`，下游副本语境）+ **非规范性 1 行**（`:69`，`## v0.9.0 变更说明`「目的」段的历史叙述，**不在判据范围**） | T4 第 1 条及其验证方法；T9 第 3 条（零改动面核对面） |
| ④ | SKILL `.pb-agents/roles`（S6 改动面） | 基线 **32 行**（出现 **35 处**）→ 改动后 **2 行** | 行数 = `grep -c`；出现次数 = `grep -o \| wc -l`（两类数字并列，**跨文件引用一律用行数口径「32 处」**） | T8 第 6 条及其验证方法 |
| ⑤ | 规范 `^### 规则` 条数 | 基线 2 → 改动后 **7** | 行数（`grep -c '^### 规则'`） | T1 验证方法 |
| ⑥ | 规范 `git -C` 命中 | 改动后 **4 处** | 行数（`grep -c 'git -C'`；= 收口三步 `:443`/`:444`/`:445` + 前置游离 `:447`） | T5 验证方法 |
| ⑦ | 术语限定面 | **7 处**（规范 6 + SKILL 1） | 行数（arch §2.3 文末唯一口径，与按出现位置计同值）；`_baseline :277` 的非术语位移另计 | T1/T4/T6/T8 各处 + T9 第 2 条 |
| ⑧ | 其余 26 处 `worktree` | **26 处**（规范 13 + SKILL 13） | = 34 − 7 − 1（arch §4-L2-7）；判定后**保持不动** | T6 第 4 条、T9 第 2 条 |
| ⑨ | 变更文件数 | **6**（5 改 + 1 新建） | `git diff --name-status`（文件行数） | 本文件头部文件范围；T9 第 5 条 |
| ⑩ | 任务数 | **9**（T1~T9） | 本文件 `## T<n> ·` 小节数 | 依赖图 / 报告契约 |
| ⑪ | `#### 判断方式` 子标题 | **0** | 行数（`grep -c '^#### 判断方式'`） | T1 验证方法、T9 第 7 条 |
| ⑫ | 规范内 `立即生效` / `追溯适用` | **0** | 行数（`grep -rn`） | T3 验证方法、T9 第 6 条 |
| ⑬ | SKILL 内 `v0.8.0` | **0** | 行数（`grep -c 'v0\.8\.0'`） | T8 第 10 条 |
| ⑭ | 本文件 `[model_inferred]` 未确认项 | **0** | 待确认条目数（2 条推导项均已转裁定登记） | 「`[model_inferred]` 验收标准」段；T2 第 7 条（位置口径）、T4 第 1 条（判据范围口径） |

> **明确排除的命中（防止再次按字面误判）**：`:69` 的 `.pb-agents/roles`（③，W1 变更说明的历史叙述）；`## 验证目标` 节的 `- **判断方式**：`（②，带 bullet 前缀、不满足行首匹配）；规则 C / 阶段 5 落点 / 规则 D 等处新增的 `.pb-agents/worktrees/`（③ 仅约束带 `/roles` 的串）；SKILL `:32` 变更历史索引行（T8 第 14 条列明保持不动）。

### 自查结论

- **依赖图**：无环（拓扑序 T1 → {T2,T3,T5,T6} → T4 → T7 → T8 → T9）。**无循环依赖，无需上报。**
- **可追溯性**：每条验收标准均带 W / S / F 编号或 arch 节号，全部落在 `architecture.md` v1.2.2 §5.1（W1~W20）/ §5.2（S1~S10）/ §2.3 / §2.4 / §3.2 / §3.3 / §7 / §8 / §5.3 / §5.4 / §5.5 与 `prd/F01~F14` 的范围内；无凭空编写的验收标准（2 条推导项均已于 2026-09-12 由主 agent 裁定并转为裁定登记，现无未确认项）。
- **覆盖度**：PR 文件「文件范围」的 6 个文件、32 条 `- [ ]` 验收条目（表为归并视图）、W1~W20（W18 为零改动行）与 S1~S10 全部有唯一承载任务（见上两张映射表）。
...
1. **`[model_inferred]` #1（位置口径）——已裁定（2026-09-12，主 agent，L3 级）**：两节落在 `### 约束生效范围` 之前还是之后，决定 T2 / T9 的核对面。现状：实现与 PR 级验收均按本任务图口径（`## 提交管理约束` 内、`### 约束生效范围` 之前）落地并通过（落地后 `:208` / `:230` < `:244`），该口径已于 2026-09-12 获主 agent 明示确认（L3 级），本项转为**裁定登记**（见「`[model_inferred]` 验收标准」段第 1 项与「计数口径定版（r2）」⑭）；**本文件现无未确认项**。
2. **`[model_inferred]` #2 已裁定（2026-09-12，`history.md` 19:52:00 条目，详见上一节第 2 项）**：判据范围限定为**规范性表述**，`## v0.9.0 变更说明` 的历史叙述（落地后 `:69`）不在其内；已写入 T4 第 1 条与「计数口径定版（r2）」③，**无需再确认**。
