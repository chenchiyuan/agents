# 阶段 6 独立验证报告 · 0028-role-model-binding

**报告文件**: `docs/iterations/0028-role-model-binding/clarifications/verify-20260916-113008.md`
**验证日期**: 2026-09-16

# 搭置记录原文（透传，不做二次判断）

> 以下为 `docs/iterations/0028-role-model-binding/deferred-demand-changes.md` 全文逐字摘录。不转述、不总结、不删减、不二次判断。
...
验证者身份：冲刺计划审查者（阶段 6 最终产物 · PR 粒度 / 依赖图 / 并发调度实证）
产出物：`/Users/chenchiyuan/projects/agents/docs/iterations/0028-role-model-binding/`（main HEAD `5995fe1`；merge `fb7b8dc`；收口登记 `3b23cba`；补录 `61601c7`；收口订正 `3b72bcb`；并发字段订正 `5995fe1`）
验证标准来源：本委托 4 项验证目标（搭置记录原文呈现 / PR 粒度三条锚点 / 依赖正确性 / 并发调度三项必查）
验证日期：2026-09-16

说明：委托中的执行过程信息不用于判定。未发起 `calls create` / 集群启停。除本报告文件外零写入。`roles/**` 用户自编辑不计入本迭代改动面。

## 逐项判定

- **1. 搭置记录原文呈现**：pass
  证据：本报告最前部一级章节「搭置记录原文（透传，不做二次判断）」为 `docs/iterations/0028-role-model-binding/deferred-demand-changes.md` 全文逐字摘录。现测：
  ```text
  $ wc -l /Users/chenchiyuan/projects/agents/docs/iterations/0028-role-model-binding/deferred-demand-changes.md
       194 /Users/chenchiyuan/projects/agents/docs/iterations/0028-role-model-binding/deferred-demand-changes.md
  ```
  摘录含 `## 需求变更` / `## 执行方式差距`（G-1~G-14、G-18、G-19、G-20）/ `## 澄清期登记的冲突`（C-1~C-4）。本节不做二次判断。

- **2. PR 粒度判断框架（10 个 PR × 逻辑原子性 / 可审查性 / 独立性）**：partial
  通过侧：10 个 PR 均满足**逻辑原子性**与**可审查性**（各只做一件可描述的事；单份 PR 文件内可完整审查，无需切换实现心智模型）。
  不通过侧：**独立性**——下列验收条目在「其他 PR 未合并」时不可判定（标准原文：验收标准可在不依赖其他 PR 合并时判断）。

  | PR | 原子性 | 可审查性 | 独立性 | 证据（文件定位） |
  |---|---|---|---|---|
  | pr-001 迭代产物入库 | pass：只入库阶段 1~3 既有正文 | pass：`git ls-files` / `--name-only` 清单 | pass：判据全在本 PR 提交面 | `prs/pr-001-iteration-artifacts-commit.md` 第 5、12–21、25–30、40–42 行 |
  | pr-002 角色级绑定 | pass：只写 `cluster.json` 两处 `model` | pass：diff/numstat 可审 | pass | `prs/pr-002-role-model-binding.md` 第 11–14、16–28、38–40 行 |
  | pr-003 一次性回执 | pass：只落三条 omp 一次性回执 | pass：命令+原始输出 | pass | `prs/pr-003-one-shot-backend-receipts.md` 第 11–13、15–23、31–33 行 |
  | pr-004 常驻探针回执 | pass：只落两条常驻回执 | pass：六字段 | **fail**：验收第 21 行「例外通道唯一性」明文消费 pr-008 的全量命令形态结论 | `prs/pr-004-resident-backend-probes.md` 第 21、33–35 行 |
  | pr-005 第二集群启动 | pass：只起第二集群并登记隔离 | pass：tmux/lsof/agents | **fail**：验收第 21 行要求副本 `roles.*.model` 与 pr-002 写入值逐字一致 | `prs/pr-005-second-cluster-bring-up.md` 第 20–21、36–38 行 |
  | pr-006 三条绑定实报 | pass：只在 7789 取三条实报 | pass：命令+信封 | pass（运行面依赖已由 `depends_on` 声明；本 PR 验收条目在本文件证据小节可核） | `prs/pr-006-second-cluster-binding-evidence.md` 第 13–28、37–39 行 |
  | pr-007 差距记录 | pass：只核补 `deferred-demand-changes.md` | pass：四要素表 | pass | `prs/pr-007-execution-gap-record.md` 第 11–14、18–27、37–39 行 |
  | pr-008 派发契约审计 | pass：只做只读全量核对 | pass：单一审计面（长但不换模型） | **fail**：验收第 26 行判定对象 = pr-006 三条 `call_id`；验收第 28 行要核 pr-007 的 G-5 | `prs/pr-008-dispatch-contract-audit.md` 第 26、28、40–42 行 |
  | pr-009 既有面冻结 | pass：只做改动面清单+复用回执 | pass | **fail**：验收第 19 行判据面含 `cluster.json`（pr-002）；验收第 21 行复用 pr-004 回执 | `prs/pr-009-existing-surface-freeze.md` 第 19、21、34–37 行 |
  | pr-010 收口生效实证 | pass：只做合入后主集群重启实证 | pass：五块证据 | **fail**：验收第 19 行要求合并后全量迭代产物可读（pr-001 建立 tracked）；验收第 21–22 行要求配置含 F03 两处 `model`（pr-002） | `prs/pr-010-post-merge-activation-evidence.md` 第 19–22、35–38 行 |

  拆开：原子性 10/10 pass；可审查性 10/10 pass；独立性 5/10 fail（pr-004 / pr-005 / pr-008 / pr-009 / pr-010）。故本项 **partial**。

- **3. 依赖正确性验证**：partial
  通过侧：依赖图无环；PR 间「文件范围」写入路径零重叠；`prd/F01`–`F13` 均被某个 PR「涉及功能点」引用。
  不通过侧：`depends_on` 中 3 条边找不到**代码级**耦合（共享符号/接口/文件），属证据消费/顺序偏好。

  **边清单（8 条）**

  | 边 | 代码级耦合证据 | 判定 |
  |---|---|---|
  | pr-005 → pr-002 | `cluster.json` 的 `roles.<role>.model` 是 `oamp/src/cluster.js:200-201` `planWindows` 追加 `--model` 的唯一来源；副本定义为该文件逐字复制 | 合规（文件引用 + 符号 `entry.model`） |
  | pr-006 → pr-005 | `oamp/src/cluster.js:190` `--port` 取 `config.web.port`（只存在于 pr-005 的 `cluster.second.json`）；`oamp/src/config.js:154` `dbPath` 按包根推导 | 合规 |
  | pr-008 → pr-006 | 理由引用 `architecture.md` §4 A-05 与 pr-006「验收证据」中的 `call_id`，**无**共享源码符号/接口 | **不合规（非代码级）** |
  | pr-004 → pr-008 | 理由为「消费 pr-008 全量命令形态核对结论」；`prd/F02` 与 `prd/F09` 同判据面，**无**代码耦合 | **不合规（非代码级）** |
  | pr-009 → pr-002 | `prd/F13` 验收 1 路径清单首项 `cluster.json`，该文件改动唯一来源 pr-002 | 合规（文件引用） |
  | pr-009 → pr-004 | 复用 pr-004「验收证据」六字段回执，**无**代码耦合 | **不合规（非代码级）** |
  | pr-010 → pr-001 | 未跟踪文件不参与 merge（`git status --porcelain` 报 `??`）；属 git 跟踪面，不是运行时符号 | 合规（文件/跟踪面引用） |
  | pr-010 → pr-002 | `oamp/src/cluster.js:200-201` `if (entry.model !== undefined) argv.push('--model', entry.model)` | 合规 |

  **无环**：边为 `002→005→006→008→004→009`，另 `002→009`、`001→010`、`002→010`。无回边。无依赖者：pr-001 / pr-002 / pr-003 / pr-007。

  **文件范围零重叠（写入声明）**

  | PR | 声明写入 |
  |---|---|
  | pr-001 | `demand.md` / `prd.md` / `prd/F01–F13` / `architecture.md` / `clarifications/round-*.md` / `history.md` / `status.md` / 本 PR 文件 |
  | pr-002 | `cluster.json` + 本 PR 文件 |
  | pr-003 | 本 PR 文件 |
  | pr-004 | 本 PR 文件 |
  | pr-005 | `<工作区>/cluster.second.json`（未纳入版本控制）+ 本 PR 文件 |
  | pr-006 | 本 PR 文件 |
  | pr-007 | `deferred-demand-changes.md` + 本 PR 文件 |
  | pr-008 | 本 PR 文件 |
  | pr-009 | 本 PR 文件 |
  | pr-010 | 本 PR 文件（`status.md` 收口登记按 A-04 排除出本 PR 写入面） |

  交集为空。活文档 `status.md`/`history.md` 后续由主 agent 滚动维护，不计入其他 PR 的「文件范围」。
  ...
  **功能点覆盖**
  ...
  | F04 | pr-005 |
  | F05 / F06 / F07 | pr-006 |
  | F08 | pr-010（pr-001 亦列 F08，仅入库） |
  | F09 / F10 / F11 | pr-008 |
  | F12 | pr-007 |
  | F13 | pr-009（pr-001 亦列 F13，仅入库） |

  无遗漏。

- **4. 并发调度真实执行证据（三项必查）**：pass

  **① worktree 时间窗口重叠：pass**
  现测至少 9 个 PR worktree **此刻同时存在于磁盘**，且 pr-001/002/003 出生时间相差 ≤1s（2026-09-15 22:27:50–51），与 `history.md` 22:32 同轮三条 planner 派发一致。
  ```text
  $ git -C /Users/chenchiyuan/projects/agents worktree list
  /Users/chenchiyuan/projects/agents                                                                                                                             5995fe1 [main]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding                                                                                e66fe1f (detached HEAD)
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-001-iteration-artifacts                           f6a8ccb [feat/0028-pr-001-iteration-artifacts]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-002-role-model-binding                            e387cb4 [feat/0028-pr-002-role-model-binding]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-003-one-shot-receipts                             edba633 [feat/0028-pr-003-one-shot-receipts]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-004-resident-probes                               1cfd97b [feat/0028-pr-004-resident-probes]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-005-second-cluster                                0ed116d [feat/0028-pr-005-second-cluster]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-006-binding-evidence                              7f15f3e [feat/0028-pr-006-binding-evidence]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-007-execution-gap-record                          b86fa80 [feat/0028-pr-007-execution-gap-record]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-008-dispatch-audit                                1edc88f [feat/0028-pr-008-dispatch-audit]
  /Users/chenchiyuan/projects/agents/.pb-agents/worktrees/0028-role-model-binding/.pb-agents/worktrees/0028-pr-009-surface-freeze                                1f2c3aa [feat/0028-pr-009-surface-freeze]
  ```
  ```text
  $ stat -f '%N birth=%SB' -t '%Y-%m-%d %H:%M:%S' .../0028-pr-001-iteration-artifacts
  .../0028-pr-001-iteration-artifacts birth=2026-09-15 22:27:50
  $ stat .../0028-pr-002-role-model-binding
  ... birth=2026-09-15 22:27:51
  $ stat .../0028-pr-003-one-shot-receipts
  ... birth=2026-09-15 22:27:51
  $ stat .../0028-pr-007-execution-gap-record
  ... birth=2026-09-16 08:50:18
  ```
  `history.md` 第 156–181 行：2026-09-15 22:32 初始化并发配置并同轮派发 pr-001/002/003 的 planner（`background`）。`status.md` 台账第 70–72 行：三条 planner `call_id` 时点均为 `22:32`。pr-010 无独立 worktree（main 上执行），不否定其余 ≥2 个 worktree 重叠。

  **② 并发配置区块真实初始化与更新：pass**
  五字段在 `0e314a3`（入库快照）已全部有值，且后续提交改写了上限 / 释放次数 / 已派发总数。
  ```text
  $ git show 0e314a3:docs/iterations/0028-role-model-binding/status.md | sed -n '26,32p'
  ## 并发配置（阶段 5）
  - **起始并发数**: 3
  - **硬上限**: 5（公式 `2×起始-1`）
  - **当前有效上限**: 3
  ...
  ```
  ```text
  $ git show 3b23cba:docs/iterations/0028-role-model-binding/status.md | sed -n '28,32p'
  - **起始并发数**: 3
  - **硬上限**: 5（公式 `2×起始-1`）
  ...
  ```
  ```text
  $ git show 5995fe1:docs/iterations/0028-role-model-binding/status.md | sed -n '28,32p'
  - **起始并发数**: 3
  - **硬上限**: 5（公式 `2×起始-1`）
  ...
  ```
  `git log --format='%h %ad %s' --date=iso -- …/status.md` 含 `0e314a3` → `3b23cba` → `3b72bcb` → `5995fe1`。不是初始化后冻结。`history.md` 第 149 行：23:22 「累计槛位释放 2 次，当前有效上限 5」；第 275 行：08:50 释放累计 3、公式重算上限 5。

  **③ 爬升公式真实触发：pass**
  公式 `min(起始并发数 + 累计槛位释放次数 × 起始并发数, 硬上限)`；起始 3、硬上限 5。
  | 时点 | 释放次数 | 公式值 | 文档记载的「当前有效上限」 | 证据 |
  |---|---|---|---|---|
  | 22:32 初始化 | 0 | min(3+0×3,5)=**3** | 3 | `0e314a3` status 第 29 行；`history.md` 第 158 行 |
  | 23:22 两条 FAIL | 2 | min(3+2×3,5)=**5** | 5 | `status.md` 更新日志第 149 行 |
  | 08:50 pr-003 合并 | 3 | min(3+3×3,5)=**5** | 5 | `history.md` 第 275 行原文 `min(3 + 3×3, 5) = 5` |
  | HEAD | 10 | min(3+10×3,5)=**5** | 5 | `5995fe1` status 第 30 行 |

  「当前有效上限」确曾从 **3 改写为 5**（`0e314a3` vs 此后各提交）。达硬上限后保持 5，与公式一致。

## 汇总

- pass: 2 项（第 1、第 4）
- fail: 0 项（需返工）
- partial: 2 项（第 2、第 3；部分需返工）
- blocked: 0 项
...
| 规格/文档描述 | 实现实际行为 | 建议处理 |
|---|---|---|
| `depends_on` 须有代码级耦合 | pr-004→pr-008、pr-008→pr-006、pr-009→pr-004 只有证据消费，无共享符号/接口 | 重新评估实现：删边或改判「调度顺序」而非依赖 |
| pr-008 验收第 28 行要核 pr-007 的 G-5 | `depends_on` 未列 pr-007 | 按实现更新文档 / 开新迭代处理 |
| `3b23cba` 的公式写作 `min(3 + 3×3, 5)` 同时「释放次数: 10」 | 释放 10 时正确式为 `min(3+10×3,5)`；`5995fe1` 已改公式与「已派发总数 10」，但 `3b23cba` 历史快照仍自相矛盾 | 按实现更新文档（已部分订正；历史提交保留） |
| `status.md:56` 写差距记录「G-1~G-10」 | 文件现含 G-1~G-14、G-18、G-19、G-20（编号不连续） | 按实现更新文档 |
| Gate 报告 `clarifications/verify-20260915-221935.md` 已记独立性 partial | 阶段 5 未改 PR `depends_on` 字面，本轮复现同一独立性缺口 | 开新迭代处理 |

## 下一迭代候选

- tmux `has-session` 前缀匹配（搭置记录 G-18）：`cluster up` 对同前缀 session 假阳性 exit 0。
- 自身实例 re-entrant 死锁与无 `calls cancel`（G-19）。
- 等待监控只覆盖终态不覆盖卡死（G-20）。
- 调用面不持久：集群重建后 `calls get` 404（G-14/G-15 族）；「`call_id` 可读」需定义历史保留期。
- 节点侧 30 分钟硬上限、调用面不透传 `timeout_ms`（G-9）。
- 探针 per-call `model` 固化常驻会话（G-11）。
- PR worktree 拉出时迭代产物 untracked（G-12）与 `*-tasks.md` 无文件范围（G-13）。
- `feat/0028-pr-*` 分支与 9 个 PR worktree 收口后仍在磁盘；`iteration/0028-role-model-binding` 已删。
- 证据消费型 `depends_on` 与「代码级耦合」标准不一致——流程需区分「合并前置」与「代码依赖」。

## 结论

**PASS**（2 pass / 2 partial / 0 fail / 0 blocked）

偏差 5 条不影响结论。主 agent 可关闭阶段 6，无需因 fail 返工。partial 为：① 5 个 PR 的独立性锚点不满足；② 3 条 `depends_on` 边无代码级耦合。
