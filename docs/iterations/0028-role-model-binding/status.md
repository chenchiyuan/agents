...
### 结项期修正（主 agent，2026-09-16 11:35）

- **差距记录合并丢失已修复**：迭代分支合入 main 时带过去的是 pr-007 的版本，而**主 agent 在迭代工作区的对账内容未被提交** ⇒ `deferred-demand-changes.md` 曾缺 3 条（G-14「被回调」/ G-16「A-01 落点 vs 规则 F」/ G-17「UDS 上限 + A-01 否决依据更正」）且编号跳号。已按并集补齐 ⇒ 现为 **G-1~G-20（连续）+ C-1~C-4**，共 24 条 / 212 行（提交信息见 `git log`）。
- **编号影响的已知不一致**：pr-007 的等价性缺口条目在补齐后为 **G-15**（其 PR 文件亦建议此编号），而**阶段 6 报告**（`clarifications/verify-20260916-113008.md`）转录的是修复前版本（当时该条为 G-14、且缺上述 3 条）。以本文件为权威，阶段 6 报告的差距转录**少 3 条**，已在此登记。
- 该缺陷与 **G-13**（活文档在迭代工作区为 untracked、合并时与 PR 分支快照对撞）同源：**本迭代的活文档对账只发生在工作区、未随分支落盘**。下迭代候选：把"活文档以迭代工作区版本为准"的对账动作改为**合并前先落盘一次**（或由收口提交统一带入并核对条目集合）。
