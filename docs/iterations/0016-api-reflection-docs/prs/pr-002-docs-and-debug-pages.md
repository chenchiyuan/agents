...
- [ ] `node --test oamp/test/api-pages.test.js` 全绿（两个新页面与顶栏入口的静态契约 + `/docs`、`/debug`、`/api-pages.css`、`/docs.js`、`/debug.js` 的 HTTP 可达性与响应类型；`/llms.txt` 的全部断言归 pr-001 的 `test/api-routes.test.js`）。
