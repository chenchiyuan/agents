...
- **指标**：每个 PR 关联的预估 diff < 400 行
- **判断方式**：verifier 对 PR 文件中声明的文件范围进行行数估算，判断合理性
- **独立性要求**：每个 PR 的验收标准必须可独立判断，不依赖其他 PR 完成
...
- PR 粒度约束（400 行）导致 commit-planner 产出与 dev 实际工作量不匹配 → 调整粒度指标
