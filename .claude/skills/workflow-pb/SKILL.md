...
name: workflow-pb
description: |
  启动并驱动 pb 产品研发工作流（v0.10.0）。主 agent 调度协议：按 6 阶段序列推进、
...
**版本**: 1.13.0（对应规范 workflow-pb v0.10.0）
...
接收迭代 ID，按 workflow-pb v0.10.0 规范调度 6 个阶段，守住每个阶段的出口定义，维护 `status.md` 进度视图，直到所有 PR 合并完成。
...
**每次推进前**：读取 `{角色定义根}/workflow-pb/workflow-pb.md` §阶段定义，逐项核查该阶段"推进条件"列，全部通过才更新 status.md 推进；未全部通过 → 回到执行角色补充，不跳过。
